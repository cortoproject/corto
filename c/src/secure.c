...
corto_secure_lock _corto_secure_lock__create(corto_object _parent, const char *_id, corto_query* query, int16_t priority) {
    ...
    if (!corto_check_state(_this, CORTO_VALID)) {
        if (query) {
            corto_ptr_copy(&((corto_secure_lock)_this)->query, corto_query_o, query);
        }
...
corto_int16 _corto_secure_lock__update(corto_secure_lock _this, corto_query* query, int16_t priority) {
    CORTO_UNUSED(_this);
    if (!corto_update_begin(_this)) {
        if ((corto_typeof(corto_typeof(_this)) == (corto_type)corto_target_o) && !corto_owned(_this)) {
            if (query) {
                corto_ptr_copy(&((corto_secure_lock)((corto_secure_lock)CORTO_OFFSET(_this, ((corto_type)corto_secure_lock_o)->size)))->query, corto_query_o, query);
            }
...
corto_secure_lock _corto_secure_lock__assign(corto_secure_lock _this, corto_query* query, int16_t priority) {
    CORTO_UNUSED(_this);
    if (query) {
        corto_ptr_copy(&((corto_secure_lock)_this)->query, corto_query_o, query);
    }
