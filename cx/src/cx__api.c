...
int cx_array__define(cx_array _this, cx_typedef elementType, cx_uint32 max) {
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794190>.elementType") : 0; cx_collection(_this)->elementType = elementType;
...
int cx_callback__define(cx_callback _this, cx_typedef returnType, cx_bool returnsReference, cx_delegate delegate) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x87941e8>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
    delegate ? cx_keep_ext(_this, delegate, "element <0x87941e8>.delegate") : 0; _this->delegate = delegate;
    return cx_define(_this);
}
...
cx_callback cx_callback__create(cx_typedef returnType, cx_bool returnsReference, cx_delegate delegate) {
    ...
    returnType ? cx_keep_ext(_this, returnType, "element <0x8794260>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
    delegate ? cx_keep_ext(_this, delegate, "element <0x8794260>.delegate") : 0; _this->delegate = delegate;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
    }
...
int cx_class__define(cx_class _this, cx_interface base, cx_modifier baseAccess, cx_interfaceSeq implements) {
    base ? cx_keep_ext(_this, base, "element <0x8794190>.base") : 0; cx_interface(_this)->base = base;
...
cx_class cx_class__create(cx_interface base, cx_modifier baseAccess, cx_interfaceSeq implements) {
    ...
    base ? cx_keep_ext(_this, base, "element <0x87942f0>.base") : 0; cx_interface(_this)->base = base;
    cx_struct(_this)->baseAccess = baseAccess;
    _this->implements = implements;
    if (cx_define(_this)) {
        cx_free(_this);
...
int cx_collection__define(cx_collection _this, cx_typedef elementType, cx_uint32 max) {
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794190>.elementType") : 0; _this->elementType = elementType;
    _this->max = max;
    return cx_define(_this);
}
...
cx_collection cx_collection__create(cx_typedef elementType, cx_uint32 max) {
    ...
    elementType ? cx_keep_ext(_this, elementType, "element <0x87941c8>.elementType") : 0; _this->elementType = elementType;
    _this->max = max;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
...
int cx_delegate__define(cx_delegate _this, cx_typedef returnType, cx_bool returnsReference) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x8793fa8>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
    return cx_define(_this);
}
...
int cx_function__define(cx_function _this, cx_typedef returnType, cx_bool returnsReference) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x8794218>.returnType") : 0; _this->returnType = returnType;
...
int cx_interface__define(cx_interface _this, cx_interface base) {
    base ? cx_keep_ext(_this, base, "element <0x87941f0>.base") : 0; _this->base = base;
    return cx_define(_this);
}
...
cx_interface cx_interface__create(cx_interface base) {
    ...
    base ? cx_keep_ext(_this, base, "element <0x8794250>.base") : 0; _this->base = base;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
    }
...
void cx_interfaceVector__init(cx_interfaceVector *_this, cx_interface interface, cx_vtable vector) {
    ...
    cx_initValue(&v);
    interface ? cx_keep_ext(_this, interface, "element <0x8793f80>.interface") : 0; _this->interface = interface;
    _this->vector = vector;
}
...
int cx_list__define(cx_list _this, cx_typedef elementType, cx_uint32 max) {
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794200>.elementType") : 0; cx_collection(_this)->elementType = elementType;
    cx_collection(_this)->max = max;
    return cx_define(_this);
}
...
int cx_map__define(cx_map _this, cx_typedef elementType, cx_typedef keyType, cx_uint32 max) {
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794200>.elementType") : 0; _this->elementType = elementType;
    keyType ? cx_keep_ext(_this, keyType, "element <0x8794200>.keyType") : 0; _this->keyType = keyType;
    _this->max = max;
    return cx_define(_this);
}
...
cx_map cx_map__create(cx_typedef elementType, cx_typedef keyType, cx_uint32 max) {
    ...
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794190>.elementType") : 0; _this->elementType = elementType;
    keyType ? cx_keep_ext(_this, keyType, "element <0x8794190>.keyType") : 0; _this->keyType = keyType;
    _this->max = max;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
...
int cx_member__define(cx_member _this, cx_typedef type, cx_modifier modifiers, cx_state state, cx_bool weak) {
    type ? cx_keep_ext(_this, type, "element <0x8794270>.type") : 0; _this->type = type;
...
int cx_metaprocedure__define(cx_metaprocedure _this, cx_typedef returnType, cx_bool returnsReference, cx_bool referenceOnly) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x87941c8>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
...
int cx_observableEvent__define(cx_observableEvent _this, cx_uint16 kind, cx_observer observer, cx_object me, cx_object source, cx_object observable) {
    ...
    observer ? cx_keep_ext(_this, observer, "element <0x8794190>.observer") : 0; _this->observer = observer;
    me ? cx_keep_ext(_this, me, "element <0x8794190>.me") : 0; _this->me = me;
    source ? cx_keep_ext(_this, source, "element <0x8794190>.source") : 0; _this->source = source;
    observable ? cx_keep_ext(_this, observable, "element <0x8794190>.observable") : 0; _this->observable = observable;
    return cx_define(_this);
}
...
int cx_observer__define(cx_observer _this, cx_object observable, cx_eventMask mask, cx_string expression, cx_uint32 template, cx_dispatcher dispatcher, cx_object me) {
    observable ? cx_keep_ext(_this, observable, "element <0x87941c8>.observable") : 0; _this->observable = observable;
    _this->mask = mask;
    _this->expression = (expression ? cx_strdup(expression) : NULL);
    _this->template = template;
    dispatcher ? cx_keep_ext(_this, dispatcher, "element <0x87941c8>.dispatcher") : 0; _this->dispatcher = dispatcher;
    me ? cx_keep_ext(_this, me, "element <0x87941c8>.me") : 0; _this->me = me;
    return cx_define(_this);
}
...
void cx_parameter__init(cx_parameter *_this, cx_string name, cx_typedef type, cx_bool passByReference) {
    ...
    type ? cx_keep_ext(_this, type, "element <0x8793fb0>.type") : 0; _this->type = type;
...
cx_procedure cx_procedure__create(cx_interface base, cx_modifier baseAccess) {
    ...
    base ? cx_keep_ext(_this, base, "element <0x87942b8>.base") : 0; cx_interface(_this)->base = base;
    cx_struct(_this)->baseAccess = baseAccess;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
...
int cx_procptr__define(cx_procptr _this, cx_typedef returnType, cx_bool returnsReference, cx_parameterSeq parameters) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x87942b8>.returnType") : 0; _this->returnType = returnType;
    _this->returnsReference = returnsReference;
...
void cx_procptrdata__init(cx_procptrdata *_this, cx_object instance, cx_function procedure) {
    ...
    cx_initValue(&v);
    instance ? cx_keep_ext(_this, instance, "element <0x8793f80>.instance") : 0; _this->instance = instance;
    procedure ? cx_keep_ext(_this, procedure, "element <0x8793f80>.procedure") : 0; _this->procedure = procedure;
}
...
int cx_sequence__define(cx_sequence _this, cx_typedef elementType, cx_uint32 max) {
    elementType ? cx_keep_ext(_this, elementType, "element <0x8794248>.elementType") : 0; cx_collection(_this)->elementType = elementType;
    cx_collection(_this)->max = max;
    return cx_define(_this);
}
...
int cx_struct__define(cx_struct _this, cx_interface base, cx_modifier baseAccess) {
    base ? cx_keep_ext(_this, base, "element <0x8794248>.base") : 0; cx_interface(_this)->base = base;
...
cx_struct cx_struct__create(cx_interface base, cx_modifier baseAccess) {
    ...
    base ? cx_keep_ext(_this, base, "element <0x8794278>.base") : 0; cx_interface(_this)->base = base;
    _this->baseAccess = baseAccess;
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
...
int cx_type__define(cx_type _this, cx_typedef defaultType, cx_typedef parentType, cx_state parentState) {
    defaultType ? cx_keep_ext(_this, defaultType, "element <0x8793f80>.defaultType") : 0; _this->defaultType = defaultType;
    parentType ? cx_keep_ext(_this, parentType, "element <0x8793f80>.parentType") : 0; _this->parentType = parentType;
...
int cx_typedef__define(cx_typedef _this, cx_typedef type) {
    type ? cx_keep_ext(_this, type, "element <0x8794190>.type") : 0; _this->type = type;
    return cx_define(_this);
}
...
int cx_virtual__define(cx_virtual _this, cx_typedef returnType, cx_bool returnsReference, cx_bool virtual) {
    returnType ? cx_keep_ext(_this, returnType, "element <0x87941c0>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
...
cx_virtual cx_virtual__create(cx_typedef returnType, cx_bool returnsReference, cx_bool virtual) {
    ...
    returnType ? cx_keep_ext(_this, returnType, "element <0x8794250>.returnType") : 0; cx_function(_this)->returnType = returnType;
    cx_function(_this)->returnsReference = returnsReference;
    cx_method(_this)->virtual = virtual;
    if (cx_define(_this)) {
        cx_free(_this);
