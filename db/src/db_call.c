...
void db_callv(db_function f, db_void* result, va_list args) {
    ...
    if(s % sizeof(db_uint64)) {
        s = ((s / sizeof(db_uint64)) + 1) * sizeof(db_uint64);
    }
    ptr = buffer = alloca(s);

    while (s) {
        *(db_uint64*)ptr = va_arg(args, db_uint64);
        ptr = (void*)((db_word)ptr + sizeof(db_uint64));
        s -= sizeof(db_uint64);
    }

    db_call_intern(f, result, buffer);
}
