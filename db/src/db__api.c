...
int db_array__define(db_array _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x8c653a0>.elementType") : 0; db_collection(_this)->elementType = elementType;
...
int db_callback__define(db_callback _this, db_typedef returnType, db_bool returnsReference, db_delegate delegate) {
    returnType ? db_keep_ext(_this, returnType, "element <0x8c56f78>.returnType") : 0; db_function(_this)->returnType = returnType;
    db_function(_this)->returnsReference = returnsReference;
    delegate ? db_keep_ext(_this, delegate, "element <0x8c56f78>.delegate") : 0; _this->delegate = delegate;
    return db_define(_this);
}
...
int db_class__define(db_class _this, db_interface base, db_modifier baseAccess, db_interfaceSeq implements) {
    base ? db_keep_ext(_this, base, "element <0x8c65600>.base") : 0; db_interface(_this)->base = base;
...
int db_collection__define(db_collection _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x8c65b90>.elementType") : 0; _this->elementType = elementType;
    _this->max = max;
    return db_define(_this);
}
...
int db_function__define(db_function _this, db_typedef returnType, db_bool returnsReference) {
    returnType ? db_keep_ext(_this, returnType, "element <0x8c56f78>.returnType") : 0; _this->returnType = returnType;
...
int db_interface__define(db_interface _this, db_interface base) {
    base ? db_keep_ext(_this, base, "element <0x8c65b90>.base") : 0; _this->base = base;
    return db_define(_this);
}
...
int db_list__define(db_list _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x8c65b90>.elementType") : 0; db_collection(_this)->elementType = elementType;
    db_collection(_this)->max = max;
    return db_define(_this);
}
...
int db_map__define(db_map _this, db_typedef elementType, db_typedef keyType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x8c65b90>.elementType") : 0; _this->elementType = elementType;
    keyType ? db_keep_ext(_this, keyType, "element <0x8c65b90>.keyType") : 0; _this->keyType = keyType;
    _this->max = max;
    return db_define(_this);
}
...
int db_member__define(db_member _this, db_typedef type, db_modifier modifiers, db_state state, db_bool weak) {
    type ? db_keep_ext(_this, type, "element <0x8c65330>.type") : 0; _this->type = type;
...
int db_observableEvent__define(db_observableEvent _this, db_uint16 kind, db_observer observer, db_object me, db_object source, db_object observable) {
    ...
    observer ? db_keep_ext(_this, observer, "element <0x8c48b88>.observer") : 0; _this->observer = observer;
    me ? db_keep_ext(_this, me, "element <0x8c48b88>.me") : 0; _this->me = me;
    source ? db_keep_ext(_this, source, "element <0x8c48b88>.source") : 0; _this->source = source;
    observable ? db_keep_ext(_this, observable, "element <0x8c48b88>.observable") : 0; _this->observable = observable;
    return db_define(_this);
}
...
int db_observer__define(db_observer _this, db_object observable, db_eventMask mask, db_string expression, db_uint32 template, db_dispatcher dispatcher, db_object me) {
    observable ? db_keep_ext(_this, observable, "element <0x8c65ba0>.observable") : 0; _this->observable = observable;
    _this->mask = mask;
    _this->expression = (expression ? db_strdup(expression) : NULL);
    _this->template = template;
    dispatcher ? db_keep_ext(_this, dispatcher, "element <0x8c65ba0>.dispatcher") : 0; _this->dispatcher = dispatcher;
    me ? db_keep_ext(_this, me, "element <0x8c65ba0>.me") : 0; _this->me = me;
    return db_define(_this);
}
...
int db_sequence__define(db_sequence _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x8c65ba0>.elementType") : 0; db_collection(_this)->elementType = elementType;
    db_collection(_this)->max = max;
    return db_define(_this);
}
...
int db_type__define(db_type _this, db_typedef defaultType, db_typedef parentType, db_state parentState) {
    defaultType ? db_keep_ext(_this, defaultType, "element <0x8c65ba0>.defaultType") : 0; _this->defaultType = defaultType;
    parentType ? db_keep_ext(_this, parentType, "element <0x8c65ba0>.parentType") : 0; _this->parentType = parentType;
...
int db_typedef__define(db_typedef _this, db_typedef type) {
    type ? db_keep_ext(_this, type, "element <0x8c653f8>.type") : 0; _this->type = type;
    return db_define(_this);
}
