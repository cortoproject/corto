...
int db_array__define(db_array _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea7c8>.elementType") : 0; db_collection(_this)->elementType = elementType;
...
db_array db_array__create(db_typedef elementType, db_uint32 max) {
    ...
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea7e8>.elementType") : 0; db_collection(_this)->elementType = elementType;
    db_collection(_this)->max = max;
    if (db_define(_this)) {
        db_free(_this);
        _this = NULL;
...
int db_callback__define(db_callback _this, db_typedef returnType, db_bool returnsReference, db_delegate delegate) {
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea870>.returnType") : 0; db_function(_this)->returnType = returnType;
    db_function(_this)->returnsReference = returnsReference;
    delegate ? db_keep_ext(_this, delegate, "element <0x89ea870>.delegate") : 0; _this->delegate = delegate;
    return db_define(_this);
}
...
int db_class__define(db_class _this, db_interface base, db_modifier baseAccess, db_interfaceSeq implements) {
    base ? db_keep_ext(_this, base, "element <0x89ea8e0>.base") : 0; db_interface(_this)->base = base;
...
db_class db_class__create(db_interface base, db_modifier baseAccess, db_interfaceSeq implements) {
    ...
    base ? db_keep_ext(_this, base, "element <0x89ea8f0>.base") : 0; db_interface(_this)->base = base;
    db_struct(_this)->baseAccess = baseAccess;
    _this->implements = implements;
    if (db_define(_this)) {
        db_free(_this);
...
int db_collection__define(db_collection _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea7c8>.elementType") : 0; _this->elementType = elementType;
    _this->max = max;
    return db_define(_this);
}
...
db_collection db_collection__create(db_typedef elementType, db_uint32 max) {
    ...
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea8e0>.elementType") : 0; _this->elementType = elementType;
    _this->max = max;
    if (db_define(_this)) {
        db_free(_this);
        _this = NULL;
...
int db_delegate__define(db_delegate _this, db_typedef returnType, db_bool returnsReference) {
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea7c8>.returnType") : 0; db_function(_this)->returnType = returnType;
    db_function(_this)->returnsReference = returnsReference;
    return db_define(_this);
}
...
int db_function__define(db_function _this, db_typedef returnType, db_bool returnsReference) {
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea850>.returnType") : 0; _this->returnType = returnType;
...
int db_interface__define(db_interface _this, db_interface base) {
    base ? db_keep_ext(_this, base, "element <0x89ea828>.base") : 0; _this->base = base;
    return db_define(_this);
}
...
db_interface db_interface__create(db_interface base) {
    ...
    base ? db_keep_ext(_this, base, "element <0x89ea888>.base") : 0; _this->base = base;
    if (db_define(_this)) {
        db_free(_this);
        _this = NULL;
    }
...
void db_interfaceVector__init(db_interfaceVector *_this, db_interface interface, db_vtable vector) {
    interface ? db_keep_ext(_this, interface, "element <0x89ea7c8>.interface") : 0; _this->interface = interface;
    _this->vector = vector;
}
...
int db_list__define(db_list _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea7f0>.elementType") : 0; db_collection(_this)->elementType = elementType;
    db_collection(_this)->max = max;
    return db_define(_this);
}
...
int db_map__define(db_map _this, db_typedef elementType, db_typedef keyType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea880>.elementType") : 0; _this->elementType = elementType;
    keyType ? db_keep_ext(_this, keyType, "element <0x89ea880>.keyType") : 0; _this->keyType = keyType;
    _this->max = max;
    return db_define(_this);
}
...
db_map db_map__create(db_typedef elementType, db_typedef keyType, db_uint32 max) {
    ...
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea7c8>.elementType") : 0; _this->elementType = elementType;
    keyType ? db_keep_ext(_this, keyType, "element <0x89ea7c8>.keyType") : 0; _this->keyType = keyType;
    _this->max = max;
    if (db_define(_this)) {
        db_free(_this);
        _this = NULL;
...
int db_member__define(db_member _this, db_typedef type, db_modifier modifiers, db_state state, db_bool weak) {
    type ? db_keep_ext(_this, type, "element <0x89ea880>.type") : 0; _this->type = type;
...
int db_observableEvent__define(db_observableEvent _this, db_uint16 kind, db_observer observer, db_object me, db_object source, db_object observable) {
    ...
    observer ? db_keep_ext(_this, observer, "element <0x89ea8c0>.observer") : 0; _this->observer = observer;
    me ? db_keep_ext(_this, me, "element <0x89ea8c0>.me") : 0; _this->me = me;
    source ? db_keep_ext(_this, source, "element <0x89ea8c0>.source") : 0; _this->source = source;
    observable ? db_keep_ext(_this, observable, "element <0x89ea8c0>.observable") : 0; _this->observable = observable;
    return db_define(_this);
}
...
int db_observer__define(db_observer _this, db_object observable, db_eventMask mask, db_string expression, db_uint32 template, db_dispatcher dispatcher, db_object me) {
    observable ? db_keep_ext(_this, observable, "element <0x89ea7c8>.observable") : 0; _this->observable = observable;
    _this->mask = mask;
    _this->expression = (expression ? db_strdup(expression) : NULL);
    _this->template = template;
    dispatcher ? db_keep_ext(_this, dispatcher, "element <0x89ea7c8>.dispatcher") : 0; _this->dispatcher = dispatcher;
    me ? db_keep_ext(_this, me, "element <0x89ea7c8>.me") : 0; _this->me = me;
    return db_define(_this);
}
...
void db_parameter__init(db_parameter *_this, db_string name, db_typedef type, db_bool passByReference) {
    ...
    type ? db_keep_ext(_this, type, "element <0x89ea7c8>.type") : 0; _this->type = type;
...
int db_procptr__define(db_procptr _this, db_typedef returnType, db_bool returnsReference, db_parameterSeq parameters) {
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea7c8>.returnType") : 0; _this->returnType = returnType;
    _this->returnsReference = returnsReference;
...
db_procptr db_procptr__create(db_typedef returnType, db_bool returnsReference, db_parameterSeq parameters) {
    ...
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea938>.returnType") : 0; _this->returnType = returnType;
    _this->returnsReference = returnsReference;
    _this->parameters = parameters;
    if (db_define(_this)) {
        db_free(_this);
...
void db_procptrdata__init(db_procptrdata *_this, db_object instance, db_function procedure) {
    instance ? db_keep_ext(_this, instance, "element <0x89ea5e8>.instance") : 0; _this->instance = instance;
    procedure ? db_keep_ext(_this, procedure, "element <0x89ea5e8>.procedure") : 0; _this->procedure = procedure;
}
...
int db_sequence__define(db_sequence _this, db_typedef elementType, db_uint32 max) {
    elementType ? db_keep_ext(_this, elementType, "element <0x89ea808>.elementType") : 0; db_collection(_this)->elementType = elementType;
    db_collection(_this)->max = max;
    return db_define(_this);
}
...
int db_struct__define(db_struct _this, db_interface base, db_modifier baseAccess) {
    base ? db_keep_ext(_this, base, "element <0x89ea7c8>.base") : 0; db_interface(_this)->base = base;
...
int db_type__define(db_type _this, db_typedef defaultType, db_typedef parentType, db_state parentState) {
    defaultType ? db_keep_ext(_this, defaultType, "element <0x89ea820>.defaultType") : 0; _this->defaultType = defaultType;
    parentType ? db_keep_ext(_this, parentType, "element <0x89ea820>.parentType") : 0; _this->parentType = parentType;
...
db_type db_type__create(db_typedef defaultType, db_typedef parentType, db_state parentState) {
    ...
    defaultType ? db_keep_ext(_this, defaultType, "element <0x89ea5b8>.defaultType") : 0; _this->defaultType = defaultType;
    parentType ? db_keep_ext(_this, parentType, "element <0x89ea5b8>.parentType") : 0; _this->parentType = parentType;
    _this->parentState = parentState;
    if (db_define(_this)) {
        db_free(_this);
        _this = NULL;
...
int db_typedef__define(db_typedef _this, db_typedef type) {
    type ? db_keep_ext(_this, type, "element <0x89ea600>.type") : 0; _this->type = type;
    return db_define(_this);
}
...
int db_virtual__define(db_virtual _this, db_typedef returnType, db_bool returnsReference, db_bool virtual) {
    returnType ? db_keep_ext(_this, returnType, "element <0x89ea820>.returnType") : 0; db_function(_this)->returnType = returnType;
    db_function(_this)->returnsReference = returnsReference;
