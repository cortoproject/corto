...
#define corto_secure_key_logout(_this, key) ( \
    ((corto_function)corto_secure_key_login_o)->kind == CORTO_PROCEDURE_CDECL \
    ? ((void (*)(corto_object, const char *))((corto_function)((corto_interface)corto_typeof(_this))->methods.buffer[((corto_method)corto_secure_key_logout_o)->index - 1])->fptr)(corto_secure_key(_this), key) \
    : *(corto_string*)corto_invoke(((corto_interface)corto_typeof(_this))->methods.buffer[((corto_method)corto_secure_key_login_o)->index - 1], NULL, corto_secure_key(_this), key) \
    )
