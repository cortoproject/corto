...
cx_enum Fast_derefKind_o;
cx_constant (*Fast_derefKind_ByReference_o);
cx_constant (*Fast_derefKind_ByValue_o);
...
cx_member Fast_Expression_deref_o;
...
cx_function Fast_isOperatorAssignment_o;
...
cx_constant (*Fast_nodeKind_StorageExpr_o);
...
cx_member Fast_ParserDeclaration_storage_o;
...
cx_class Fast_Storage_o;
cx_method Fast_Storage_construct_o;
cx_member Fast_Storage_kind_o;
cx_enum Fast_storageKind_o;
cx_constant (*Fast_storageKind_ElementStorage_o);
cx_constant (*Fast_storageKind_LocalStorage_o);
cx_constant (*Fast_storageKind_MemberStorage_o);
cx_constant (*Fast_storageKind_ObjectStorage_o);
cx_constant (*Fast_storageKind_TemplateStorage_o);
...
int Fast_load(void) {
    ...
    /* Declare ::cortex::Fast::Binary::deref */
    Fast_Binary_deref_o = cx_declare(Fast_Binary_o, "deref", cx_type(cx_member_o));
    if (!Fast_Binary_deref_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Binary::deref'.");
        goto error;
    }
    ...
    /* Declare ::cortex::Fast::Block::isRoot */
    Fast_Block_isRoot_o = cx_declare(Fast_Block_o, "isRoot", cx_type(cx_member_o));
    if (!Fast_Block_isRoot_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Block::isRoot'.");
        goto error;
    }

    /* Define ::cortex::Fast::Block::isRoot */
    if (!cx_checkState(Fast_Block_isRoot_o, CX_DEFINED)) {
        Fast_Block_isRoot_o->type = cx_resolve_ext(Fast_Block_isRoot_o, NULL, "::cortex::lang::bool", FALSE, "element ::cortex::Fast::Block::isRoot.type");
        Fast_Block_isRoot_o->modifiers = 0x4;
        Fast_Block_isRoot_o->state = 0x6;
        Fast_Block_isRoot_o->weak = FALSE;
        Fast_Block_isRoot_o->id = 1;
        if (cx_define(Fast_Block_isRoot_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Block::isRoot'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Cast_lvalue_o, CX_DEFINED)) {
        Fast_Cast_lvalue_o->type = cx_resolve_ext(Fast_Cast_lvalue_o, NULL, "::cortex::lang::type", FALSE, "element ::cortex::Fast::Cast::lvalue.type");
    ...
    /* Declare ::cortex::Fast::derefKind */
    Fast_derefKind_o = cx_declare(Fast_o, "derefKind", cx_type(cx_enum_o));
    if (!Fast_derefKind_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::derefKind'.");
        goto error;
    }

    /* Declare ::cortex::Fast::derefKind::ByReference */
    Fast_derefKind_ByReference_o = cx_declare(Fast_derefKind_o, "ByReference", cx_type(cx_constant_o));
    if (!Fast_derefKind_ByReference_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::derefKind::ByReference'.");
        goto error;
    }

    /* Define ::cortex::Fast::derefKind::ByReference */
    if (!cx_checkState(Fast_derefKind_ByReference_o, CX_DEFINED)) {
        (*Fast_derefKind_ByReference_o) = 1;
        if (cx_define(Fast_derefKind_ByReference_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::derefKind::ByReference'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::derefKind::ByValue */
    Fast_derefKind_ByValue_o = cx_declare(Fast_derefKind_o, "ByValue", cx_type(cx_constant_o));
    if (!Fast_derefKind_ByValue_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::derefKind::ByValue'.");
        goto error;
    }

    /* Define ::cortex::Fast::derefKind::ByValue */
    if (!cx_checkState(Fast_derefKind_ByValue_o, CX_DEFINED)) {
        (*Fast_derefKind_ByValue_o) = 0;
        if (cx_define(Fast_derefKind_ByValue_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::derefKind::ByValue'.");
            goto error;
        }
    }

    /* Define ::cortex::Fast::derefKind */
    if (!cx_checkState(Fast_derefKind_o, CX_DEFINED)) {
        if (cx_define(Fast_derefKind_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::derefKind'.");
            goto error;
        }
    }

    if (cx_type(Fast_derefKind_o)->size != sizeof(Fast_derefKind)) {
        cx_error("Fast_load: calculated size '%d' of type '::cortex::Fast::derefKind' doesn't match C-type size '%d'", cx_type(Fast_derefKind_o)->size, sizeof(Fast_derefKind));
    }

    /* Define ::cortex::Fast::Binary::deref */
    if (!cx_checkState(Fast_Binary_deref_o, CX_DEFINED)) {
        Fast_Binary_deref_o->type = cx_resolve_ext(Fast_Binary_deref_o, NULL, "::cortex::Fast::derefKind", FALSE, "element ::cortex::Fast::Binary::deref.type");
        Fast_Binary_deref_o->modifiers = 0x3;
        Fast_Binary_deref_o->state = 0x6;
        Fast_Binary_deref_o->weak = FALSE;
        Fast_Binary_deref_o->id = 3;
        if (cx_define(Fast_Binary_deref_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Binary::deref'.");
            goto error;
        }
    }
    ...
    /* Declare ::cortex::Fast::Expression::deref */
    Fast_Expression_deref_o = cx_declare(Fast_Expression_o, "deref", cx_type(cx_member_o));
    if (!Fast_Expression_deref_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Expression::deref'.");
        goto error;
    }

    /* Define ::cortex::Fast::Expression::deref */
    if (!cx_checkState(Fast_Expression_deref_o, CX_DEFINED)) {
        Fast_Expression_deref_o->type = cx_resolve_ext(Fast_Expression_deref_o, NULL, "::cortex::Fast::derefKind", FALSE, "element ::cortex::Fast::Expression::deref.type");
        Fast_Expression_deref_o->modifiers = 0x4;
        Fast_Expression_deref_o->state = 0x6;
        Fast_Expression_deref_o->weak = FALSE;
        Fast_Expression_deref_o->id = 2;
        if (cx_define(Fast_Expression_deref_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Expression::deref'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Expression_type_o, CX_DEFINED)) {
        Fast_Expression_type_o->type = cx_resolve_ext(Fast_Expression_type_o, NULL, "::cortex::lang::type", FALSE, "element ::cortex::Fast::Expression::type.type");
    ...
    if (!cx_checkState(Fast_InitializerFrame_o, CX_DEFINED)) {
        ...
        cx_type(Fast_InitializerFrame_o)->parentState = 0x6;
    ...
    /* Declare ::cortex::Fast::isOperatorAssignment(operatorKind operator) */
    Fast_isOperatorAssignment_o = cx_declare(Fast_o, "isOperatorAssignment(operatorKind operator)", cx_type(cx_function_o));
    if (!Fast_isOperatorAssignment_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::isOperatorAssignment(operatorKind operator)'.");
        goto error;
    }

    /* Define ::cortex::Fast::isOperatorAssignment(operatorKind operator) */
    if (!cx_checkState(Fast_isOperatorAssignment_o, CX_DEFINED)) {
        Fast_isOperatorAssignment_o->returnType = cx_resolve_ext(Fast_isOperatorAssignment_o, NULL, "::cortex::lang::bool", FALSE, "element ::cortex::Fast::isOperatorAssignment(operatorKind operator).returnType");
        Fast_isOperatorAssignment_o->returnsReference = FALSE;
        
        /* Bind ::cortex::Fast::isOperatorAssignment(operatorKind operator) with C-function */
        cx_function(Fast_isOperatorAssignment_o)->kind = CX_PROCEDURE_CDECL;
        void __Fast_isOperatorAssignment(void *args, void *result);
        cx_function(Fast_isOperatorAssignment_o)->impl = (cx_word)__Fast_isOperatorAssignment;
        if (cx_define(Fast_isOperatorAssignment_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::isOperatorAssignment(operatorKind operator)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Local_type_o, CX_DEFINED)) {
        Fast_Local_type_o->type = cx_resolve_ext(Fast_Local_type_o, NULL, "::cortex::lang::type", FALSE, "element ::cortex::Fast::Local::type.type");
    ...
    if (!cx_checkState(Fast_New_type_o, CX_DEFINED)) {
        Fast_New_type_o->type = cx_resolve_ext(Fast_New_type_o, NULL, "::cortex::lang::type", FALSE, "element ::cortex::Fast::New::type.type");
    ...
    if (!cx_checkState(Fast_nodeKind_DeclarationExpr_o, CX_DEFINED)) {
        (*Fast_nodeKind_DeclarationExpr_o) = 4;
    ...
    if (!cx_checkState(Fast_nodeKind_InitializerExpr_o, CX_DEFINED)) {
        (*Fast_nodeKind_InitializerExpr_o) = 8;
    ...
    if (!cx_checkState(Fast_nodeKind_PostfixExpr_o, CX_DEFINED)) {
        (*Fast_nodeKind_PostfixExpr_o) = 12;
    ...
    /* Declare ::cortex::Fast::nodeKind::StorageExpr */
    Fast_nodeKind_StorageExpr_o = cx_declare(Fast_nodeKind_o, "StorageExpr", cx_type(cx_constant_o));
    if (!Fast_nodeKind_StorageExpr_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::nodeKind::StorageExpr'.");
        goto error;
    }

    /* Define ::cortex::Fast::nodeKind::StorageExpr */
    if (!cx_checkState(Fast_nodeKind_StorageExpr_o, CX_DEFINED)) {
        (*Fast_nodeKind_StorageExpr_o) = 16;
        if (cx_define(Fast_nodeKind_StorageExpr_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::nodeKind::StorageExpr'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_nodeKind_TernaryExpr_o, CX_DEFINED)) {
        (*Fast_nodeKind_TernaryExpr_o) = 13;
    ...
    /* Declare ::cortex::Fast::Object::value */
    Fast_Object_value_o = cx_declare(Fast_Object_o, "value", cx_type(cx_member_o));
    if (!Fast_Object_value_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Object::value'.");
        goto error;
    }

    /* Define ::cortex::Fast::Object::value */
    if (!cx_checkState(Fast_Object_value_o, CX_DEFINED)) {
        Fast_Object_value_o->type = cx_resolve_ext(Fast_Object_value_o, NULL, "::cortex::lang::object", FALSE, "element ::cortex::Fast::Object::value.type");
        Fast_Object_value_o->modifiers = 0x0;
        Fast_Object_value_o->state = 0x6;
        Fast_Object_value_o->weak = FALSE;
        Fast_Object_value_o->id = 0;
        if (cx_define(Fast_Object_value_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Object::value'.");
            goto error;
        }
    }
    ...
    /* Declare ::cortex::Fast::Parser::argumentToString(type type,string id,bool reference) */
    Fast_Parser_argumentToString_o = cx_declare(Fast_Parser_o, "argumentToString(type type,string id,bool reference)", cx_type(cx_method_o));
    if (!Fast_Parser_argumentToString_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::argumentToString(type type,string id,bool reference)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::argumentToString(type type,string id,bool reference) */
    if (!cx_checkState(Fast_Parser_argumentToString_o, CX_DEFINED)) {
        cx_function(Fast_Parser_argumentToString_o)->returnType = cx_resolve_ext(Fast_Parser_argumentToString_o, NULL, "::cortex::lang::string", FALSE, "element ::cortex::Fast::Parser::argumentToString(type type,string id,bool reference).returnType");
        cx_function(Fast_Parser_argumentToString_o)->returnsReference = FALSE;
        Fast_Parser_argumentToString_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::argumentToString(type type,string id,bool reference) with C-function */
        ...
        if (cx_define(Fast_Parser_argumentToString_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::argumentToString(type type,string id,bool reference)'.");
            goto error;
        }
    }
    ...
    /* Declare ::cortex::Fast::Parser::declaration(type type,string id,bool isReference) */
    Fast_Parser_declaration_o = cx_declare(Fast_Parser_o, "declaration(type type,string id,bool isReference)", cx_type(cx_method_o));
    if (!Fast_Parser_declaration_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::declaration(type type,string id,bool isReference)'.");
        goto error;
    }

    /* Declare ::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference) */
    Fast_Parser_declareFunction_o = cx_declare(Fast_Parser_o, "declareFunction(type returnType,string id,type kind,bool returnsReference)", cx_type(cx_method_o));
    if (!Fast_Parser_declareFunction_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference)'.");
        goto error;
    }
    ...
    /* Declare ::cortex::Fast::Parser::popScope(object previous) */
    Fast_Parser_popScope_o = cx_declare(Fast_Parser_o, "popScope(object previous)", cx_type(cx_method_o));
    if (!Fast_Parser_popScope_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::popScope(object previous)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::popScope(object previous) */
    if (!cx_checkState(Fast_Parser_popScope_o, CX_DEFINED)) {
        cx_function(Fast_Parser_popScope_o)->returnType = cx_resolve_ext(Fast_Parser_popScope_o, NULL, "::cortex::lang::void", FALSE, "element ::cortex::Fast::Parser::popScope(object previous).returnType");
        cx_function(Fast_Parser_popScope_o)->returnsReference = FALSE;
        Fast_Parser_popScope_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::popScope(object previous) with C-function */
        ...
        if (cx_define(Fast_Parser_popScope_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::popScope(object previous)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Parser_pushScope_o, CX_DEFINED)) {
        cx_function(Fast_Parser_pushScope_o)->returnType = cx_resolve_ext(Fast_Parser_pushScope_o, NULL, "::cortex::lang::object", FALSE, "element ::cortex::Fast::Parser::pushScope().returnType");
    ...
    if (!cx_checkState(Fast_Parser_scope_o, CX_DEFINED)) {
        Fast_Parser_scope_o->type = cx_resolve_ext(Fast_Parser_scope_o, NULL, "::cortex::lang::object", FALSE, "element ::cortex::Fast::Parser::scope.type");
    ...
    /* Declare ::cortex::Fast::ParserDeclaration::storage */
    Fast_ParserDeclaration_storage_o = cx_declare(Fast_ParserDeclaration_o, "storage", cx_type(cx_member_o));
    if (!Fast_ParserDeclaration_storage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::ParserDeclaration::storage'.");
        goto error;
    }
    ...
    /* Declare ::cortex::Fast::Storage */
    Fast_Storage_o = cx_declare(Fast_o, "Storage", cx_type(cx_class_o));
    if (!Fast_Storage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Storage'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::declaration(type type,string id,bool isReference) */
    if (!cx_checkState(Fast_Parser_declaration_o, CX_DEFINED)) {
        cx_function(Fast_Parser_declaration_o)->returnType = cx_resolve_ext(Fast_Parser_declaration_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Parser::declaration(type type,string id,bool isReference).returnType");
        cx_function(Fast_Parser_declaration_o)->returnsReference = FALSE;
        Fast_Parser_declaration_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::declaration(type type,string id,bool isReference) with C-function */
        ...
        if (cx_define(Fast_Parser_declaration_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::declaration(type type,string id,bool isReference)'.");
            goto error;
        }
    }

    /* Define ::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference) */
    if (!cx_checkState(Fast_Parser_declareFunction_o, CX_DEFINED)) {
        cx_function(Fast_Parser_declareFunction_o)->returnType = cx_resolve_ext(Fast_Parser_declareFunction_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference).returnType");
        cx_function(Fast_Parser_declareFunction_o)->returnsReference = FALSE;
        Fast_Parser_declareFunction_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference) with C-function */
        ...
        if (cx_define(Fast_Parser_declareFunction_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::declareFunction(type returnType,string id,type kind,bool returnsReference)'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::Storage::kind */
    Fast_Storage_kind_o = cx_declare(Fast_Storage_o, "kind", cx_type(cx_member_o));
    if (!Fast_Storage_kind_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Storage::kind'.");
        goto error;
    }

    /* Declare ::cortex::Fast::storageKind */
    Fast_storageKind_o = cx_declare(Fast_o, "storageKind", cx_type(cx_enum_o));
    if (!Fast_storageKind_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind'.");
        goto error;
    }

    /* Declare ::cortex::Fast::storageKind::ElementStorage */
    Fast_storageKind_ElementStorage_o = cx_declare(Fast_storageKind_o, "ElementStorage", cx_type(cx_constant_o));
    if (!Fast_storageKind_ElementStorage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind::ElementStorage'.");
        goto error;
    }

    /* Define ::cortex::Fast::storageKind::ElementStorage */
    if (!cx_checkState(Fast_storageKind_ElementStorage_o, CX_DEFINED)) {
        (*Fast_storageKind_ElementStorage_o) = 3;
        if (cx_define(Fast_storageKind_ElementStorage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind::ElementStorage'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::storageKind::LocalStorage */
    Fast_storageKind_LocalStorage_o = cx_declare(Fast_storageKind_o, "LocalStorage", cx_type(cx_constant_o));
    if (!Fast_storageKind_LocalStorage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind::LocalStorage'.");
        goto error;
    }

    /* Define ::cortex::Fast::storageKind::LocalStorage */
    if (!cx_checkState(Fast_storageKind_LocalStorage_o, CX_DEFINED)) {
        (*Fast_storageKind_LocalStorage_o) = 0;
        if (cx_define(Fast_storageKind_LocalStorage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind::LocalStorage'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::storageKind::MemberStorage */
    Fast_storageKind_MemberStorage_o = cx_declare(Fast_storageKind_o, "MemberStorage", cx_type(cx_constant_o));
    if (!Fast_storageKind_MemberStorage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind::MemberStorage'.");
        goto error;
    }

    /* Define ::cortex::Fast::storageKind::MemberStorage */
    if (!cx_checkState(Fast_storageKind_MemberStorage_o, CX_DEFINED)) {
        (*Fast_storageKind_MemberStorage_o) = 2;
        if (cx_define(Fast_storageKind_MemberStorage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind::MemberStorage'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::storageKind::ObjectStorage */
    Fast_storageKind_ObjectStorage_o = cx_declare(Fast_storageKind_o, "ObjectStorage", cx_type(cx_constant_o));
    if (!Fast_storageKind_ObjectStorage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind::ObjectStorage'.");
        goto error;
    }

    /* Define ::cortex::Fast::storageKind::ObjectStorage */
    if (!cx_checkState(Fast_storageKind_ObjectStorage_o, CX_DEFINED)) {
        (*Fast_storageKind_ObjectStorage_o) = 1;
        if (cx_define(Fast_storageKind_ObjectStorage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind::ObjectStorage'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::storageKind::TemplateStorage */
    Fast_storageKind_TemplateStorage_o = cx_declare(Fast_storageKind_o, "TemplateStorage", cx_type(cx_constant_o));
    if (!Fast_storageKind_TemplateStorage_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::storageKind::TemplateStorage'.");
        goto error;
    }

    /* Define ::cortex::Fast::storageKind::TemplateStorage */
    if (!cx_checkState(Fast_storageKind_TemplateStorage_o, CX_DEFINED)) {
        (*Fast_storageKind_TemplateStorage_o) = 4;
        if (cx_define(Fast_storageKind_TemplateStorage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind::TemplateStorage'.");
            goto error;
        }
    }

    /* Define ::cortex::Fast::storageKind */
    if (!cx_checkState(Fast_storageKind_o, CX_DEFINED)) {
        if (cx_define(Fast_storageKind_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::storageKind'.");
            goto error;
        }
    }

    if (cx_type(Fast_storageKind_o)->size != sizeof(Fast_storageKind)) {
        cx_error("Fast_load: calculated size '%d' of type '::cortex::Fast::storageKind' doesn't match C-type size '%d'", cx_type(Fast_storageKind_o)->size, sizeof(Fast_storageKind));
    }

    /* Define ::cortex::Fast::Storage::kind */
    if (!cx_checkState(Fast_Storage_kind_o, CX_DEFINED)) {
        Fast_Storage_kind_o->type = cx_resolve_ext(Fast_Storage_kind_o, NULL, "::cortex::Fast::storageKind", FALSE, "element ::cortex::Fast::Storage::kind.type");
        Fast_Storage_kind_o->modifiers = 0x0;
        Fast_Storage_kind_o->state = 0x6;
        Fast_Storage_kind_o->weak = FALSE;
        Fast_Storage_kind_o->id = 0;
        if (cx_define(Fast_Storage_kind_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Storage::kind'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_String_scope_o, CX_DEFINED)) {
        Fast_String_scope_o->type = cx_resolve_ext(Fast_String_scope_o, NULL, "::cortex::lang::object", FALSE, "element ::cortex::Fast::String::scope.type");
    ...
    if (!cx_checkState(Fast_UpdateKind_UpdateBegin_o, CX_DEFINED)) {
        (*Fast_UpdateKind_UpdateBegin_o) = 1;
    ...
    if (!cx_checkState(Fast_UpdateKind_UpdateDefault_o, CX_DEFINED)) {
        (*Fast_UpdateKind_UpdateDefault_o) = 0;
    ...
    if (!cx_checkState(Fast_Node_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Node_o)->parentState = 0x6;
    ...
    /* Declare ::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference) */
    Fast_Block_declare_o = cx_declare(Fast_Block_o, "declare(string id,type type,bool isParameter,bool isReference)", cx_type(cx_method_o));
    if (!Fast_Block_declare_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference) */
    if (!cx_checkState(Fast_Block_declare_o, CX_DEFINED)) {
        cx_function(Fast_Block_declare_o)->returnType = cx_resolve_ext(Fast_Block_declare_o, NULL, "::cortex::Fast::Local", FALSE, "element ::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference).returnType");
        cx_function(Fast_Block_declare_o)->returnsReference = FALSE;
        Fast_Block_declare_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference) with C-function */
        ...
        if (cx_define(Fast_Block_declare_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Block::declare(string id,type type,bool isParameter,bool isReference)'.");
            goto error;
        }
    }
    ...
    /* Declare ::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference) */
    Fast_Block_declareTemplate_o = cx_declare(Fast_Block_o, "declareTemplate(string id,type type,bool isParameter,bool isReference)", cx_type(cx_method_o));
    if (!Fast_Block_declareTemplate_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference) */
    if (!cx_checkState(Fast_Block_declareTemplate_o, CX_DEFINED)) {
        cx_function(Fast_Block_declareTemplate_o)->returnType = cx_resolve_ext(Fast_Block_declareTemplate_o, NULL, "::cortex::Fast::Template", FALSE, "element ::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference).returnType");
        cx_function(Fast_Block_declareTemplate_o)->returnsReference = FALSE;
        Fast_Block_declareTemplate_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference) with C-function */
        ...
        if (cx_define(Fast_Block_declareTemplate_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Block::declareTemplate(string id,type type,bool isParameter,bool isReference)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_StaticInitializerFrame_o, CX_DEFINED)) {
        ...
        cx_type(Fast_StaticInitializerFrame_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Block_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Block_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Binding_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Binding_o)->parentState = 0x6;
    ...
    /* Declare ::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column) */
    Fast_Parser_parseExpression_o = cx_declare(Fast_Parser_o, "parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column)", cx_type(cx_method_o));
    if (!Fast_Parser_parseExpression_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column) */
    if (!cx_checkState(Fast_Parser_parseExpression_o, CX_DEFINED)) {
        cx_function(Fast_Parser_parseExpression_o)->returnType = cx_resolve_ext(Fast_Parser_parseExpression_o, NULL, "::cortex::Fast::Expression", FALSE, "element ::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column).returnType");
        cx_function(Fast_Parser_parseExpression_o)->returnsReference = FALSE;
        Fast_Parser_parseExpression_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column) with C-function */
        ...
        if (cx_define(Fast_Parser_parseExpression_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::parseExpression(string expr,Fast::Block block,object scope,uint32 line,uint32 column)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Parser_stagedId_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Parser_stagedId_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Expression_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Expression_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Binary_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Binary_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Call_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Call_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_StaticCall_o, CX_DEFINED)) {
        ...
        cx_type(Fast_StaticCall_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_CallBuilder_o, CX_DEFINED)) {
        ...
        cx_type(Fast_CallBuilder_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Cast_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Cast_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Comma_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Comma_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Define_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Define_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_DelegateCall_o, CX_DEFINED)) {
        ...
        cx_type(Fast_DelegateCall_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_If_o, CX_DEFINED)) {
        ...
        cx_type(Fast_If_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_InitializerVariable_o, CX_DEFINED)) {
        ...
        cx_type(Fast_InitializerVariable_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Initializer_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Initializer_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_InitializerExpression_o, CX_DEFINED)) {
        ...
        cx_type(Fast_InitializerExpression_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_StaticInitializer_o, CX_DEFINED)) {
        ...
        cx_type(Fast_StaticInitializer_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_InitOper_o, CX_DEFINED)) {
        ...
        cx_type(Fast_InitOper_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Literal_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Literal_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Boolean_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Boolean_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Character_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Character_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_FloatingPoint_o, CX_DEFINED)) {
        ...
        cx_type(Fast_FloatingPoint_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Integer_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Integer_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_DynamicInitializerFrame_o, CX_DEFINED)) {
        ...
        cx_type(Fast_DynamicInitializerFrame_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_DynamicInitializer_o, CX_DEFINED)) {
        ...
        cx_type(Fast_DynamicInitializer_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Null_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Null_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_SignedInteger_o, CX_DEFINED)) {
        ...
        cx_type(Fast_SignedInteger_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_String_o, CX_DEFINED)) {
        ...
        cx_type(Fast_String_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Lvalue_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Lvalue_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_New_o, CX_DEFINED)) {
        ...
        cx_type(Fast_New_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Parser_binaryExpr_o, CX_DEFINED)) {
        cx_function(Fast_Parser_binaryExpr_o)->returnType = cx_resolve_ext(Fast_Parser_binaryExpr_o, NULL, "::cortex::Fast::Node", FALSE, "element ::cortex::Fast::Parser::binaryExpr(Fast::Expression lvalues,Fast::Expression rvalues,operatorKind operator).returnType");
    ...
    /* Declare ::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue) */
    Fast_Parser_castExpr_o = cx_declare(Fast_Parser_o, "castExpr(type lvalue,Fast::Expression rvalue)", cx_type(cx_method_o));
    if (!Fast_Parser_castExpr_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue) */
    if (!cx_checkState(Fast_Parser_castExpr_o, CX_DEFINED)) {
        cx_function(Fast_Parser_castExpr_o)->returnType = cx_resolve_ext(Fast_Parser_castExpr_o, NULL, "::cortex::Fast::Expression", FALSE, "element ::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue).returnType");
        cx_function(Fast_Parser_castExpr_o)->returnsReference = FALSE;
        Fast_Parser_castExpr_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue) with C-function */
        ...
        if (cx_define(Fast_Parser_castExpr_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::castExpr(type lvalue,Fast::Expression rvalue)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_ParserNew_o, CX_DEFINED)) {
        ...
        cx_type(Fast_ParserNew_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_PostFix_o, CX_DEFINED)) {
        ...
        cx_type(Fast_PostFix_o)->parentState = 0x6;
    ...
    /* Declare ::cortex::Fast::Storage::construct() */
    Fast_Storage_construct_o = cx_declare(Fast_Storage_o, "construct()", cx_type(cx_method_o));
    if (!Fast_Storage_construct_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Storage::construct()'.");
        goto error;
    }

    /* Define ::cortex::Fast::Storage::construct() */
    if (!cx_checkState(Fast_Storage_construct_o, CX_DEFINED)) {
        cx_function(Fast_Storage_construct_o)->returnType = cx_resolve_ext(Fast_Storage_construct_o, NULL, "::cortex::lang::int16", FALSE, "element ::cortex::Fast::Storage::construct().returnType");
        cx_function(Fast_Storage_construct_o)->returnsReference = FALSE;
        Fast_Storage_construct_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Storage::construct() with C-function */
        cx_function(Fast_Storage_construct_o)->kind = CX_PROCEDURE_CDECL;
        void __Fast_Storage_construct(void *args, void *result);
        cx_function(Fast_Storage_construct_o)->impl = (cx_word)__Fast_Storage_construct;
        if (cx_define(Fast_Storage_construct_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Storage::construct()'.");
            goto error;
        }
    }

    /* Define ::cortex::Fast::Storage */
    if (!cx_checkState(Fast_Storage_o, CX_DEFINED)) {
        cx_type(Fast_Storage_o)->defaultType = cx_resolve_ext(Fast_Storage_o, NULL, "::cortex::lang::member", FALSE, "element ::cortex::Fast::Storage.defaultType");
        cx_type(Fast_Storage_o)->parentType = NULL;
        cx_type(Fast_Storage_o)->parentState = 0x6;
        cx_interface(Fast_Storage_o)->base = cx_resolve_ext(Fast_Storage_o, NULL, "::cortex::Fast::Expression", FALSE, "element ::cortex::Fast::Storage.base");
        cx_struct(Fast_Storage_o)->baseAccess = 0x4;
        Fast_Storage_o->implements.length = 0;
        Fast_Storage_o->implements.buffer = NULL;
        if (cx_define(Fast_Storage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Storage'.");
            goto error;
        }
    }

    if (cx_type(Fast_Storage_o)->size != sizeof(struct Fast_Storage_s)) {
        cx_error("Fast_load: calculated size '%d' of type '::cortex::Fast::Storage' doesn't match C-type size '%d'", cx_type(Fast_Storage_o)->size, sizeof(struct Fast_Storage_s));
    }
    ...
    if (!cx_checkState(Fast_Element_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Element_o)->parentState = 0x6;
        cx_interface(Fast_Element_o)->base = cx_resolve_ext(Fast_Element_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Element.base");
    ...
    if (!cx_checkState(Fast_Local_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Local_o)->parentState = 0x6;
        cx_interface(Fast_Local_o)->base = cx_resolve_ext(Fast_Local_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Local.base");
    ...
    if (!cx_checkState(Fast_Template_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Template_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Member_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Member_o)->parentState = 0x6;
        cx_interface(Fast_Member_o)->base = cx_resolve_ext(Fast_Member_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Member.base");
    ...
    if (!cx_checkState(Fast_Object_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Object_o)->parentState = 0x6;
        cx_interface(Fast_Object_o)->base = cx_resolve_ext(Fast_Object_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Object.base");
        cx_struct(Fast_Object_o)->baseAccess = 0x4;
    ...
    if (!cx_checkState(Fast_Parser_observerDeclaration_o, CX_DEFINED)) {
        cx_function(Fast_Parser_observerDeclaration_o)->returnType = cx_resolve_ext(Fast_Parser_observerDeclaration_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::Parser::observerDeclaration(string id,Fast::Expression object,eventMask mask,Fast::Object dispatcher).returnType");
    ...
    /* Declare ::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block) */
    Fast_Parser_bind_o = cx_declare(Fast_Parser_o, "bind(Fast::Storage function,Fast::Block block)", cx_type(cx_method_o));
    if (!Fast_Parser_bind_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block) */
    if (!cx_checkState(Fast_Parser_bind_o, CX_DEFINED)) {
        cx_function(Fast_Parser_bind_o)->returnType = cx_resolve_ext(Fast_Parser_bind_o, NULL, "::cortex::lang::int16", FALSE, "element ::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block).returnType");
        cx_function(Fast_Parser_bind_o)->returnsReference = FALSE;
        Fast_Parser_bind_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block) with C-function */
        ...
        if (cx_define(Fast_Parser_bind_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::bind(Fast::Storage function,Fast::Block block)'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr) */
    Fast_Parser_bindOneliner_o = cx_declare(Fast_Parser_o, "bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr)", cx_type(cx_method_o));
    if (!Fast_Parser_bindOneliner_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr) */
    if (!cx_checkState(Fast_Parser_bindOneliner_o, CX_DEFINED)) {
        cx_function(Fast_Parser_bindOneliner_o)->returnType = cx_resolve_ext(Fast_Parser_bindOneliner_o, NULL, "::cortex::lang::int16", FALSE, "element ::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr).returnType");
        cx_function(Fast_Parser_bindOneliner_o)->returnsReference = FALSE;
        Fast_Parser_bindOneliner_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr) with C-function */
        ...
        if (cx_define(Fast_Parser_bindOneliner_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::bindOneliner(Fast::Storage function,Fast::Block block,Fast::Expression expr)'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::Parser::declareFunctionParams(Storage function) */
    Fast_Parser_declareFunctionParams_o = cx_declare(Fast_Parser_o, "declareFunctionParams(Storage function)", cx_type(cx_method_o));
    if (!Fast_Parser_declareFunctionParams_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::declareFunctionParams(Storage function)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::declareFunctionParams(Storage function) */
    if (!cx_checkState(Fast_Parser_declareFunctionParams_o, CX_DEFINED)) {
        cx_function(Fast_Parser_declareFunctionParams_o)->returnType = cx_resolve_ext(Fast_Parser_declareFunctionParams_o, NULL, "::cortex::Fast::Block", FALSE, "element ::cortex::Fast::Parser::declareFunctionParams(Storage function).returnType");
        cx_function(Fast_Parser_declareFunctionParams_o)->returnsReference = FALSE;
        Fast_Parser_declareFunctionParams_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::declareFunctionParams(Storage function) with C-function */
        ...
        if (cx_define(Fast_Parser_declareFunctionParams_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::declareFunctionParams(Storage function)'.");
            goto error;
        }
    }

    /* Declare ::cortex::Fast::Parser::defineVariable(Storage object) */
    Fast_Parser_defineVariable_o = cx_declare(Fast_Parser_o, "defineVariable(Storage object)", cx_type(cx_method_o));
    if (!Fast_Parser_defineVariable_o) {
        cx_error("Fast_load: failed to declare object '::cortex::Fast::Parser::defineVariable(Storage object)'.");
        goto error;
    }

    /* Define ::cortex::Fast::Parser::defineVariable(Storage object) */
    if (!cx_checkState(Fast_Parser_defineVariable_o, CX_DEFINED)) {
        cx_function(Fast_Parser_defineVariable_o)->returnType = cx_resolve_ext(Fast_Parser_defineVariable_o, NULL, "::cortex::lang::int16", FALSE, "element ::cortex::Fast::Parser::defineVariable(Storage object).returnType");
        cx_function(Fast_Parser_defineVariable_o)->returnsReference = FALSE;
        Fast_Parser_defineVariable_o->virtual = FALSE;
        
        /* Bind ::cortex::Fast::Parser::defineVariable(Storage object) with C-function */
        ...
        if (cx_define(Fast_Parser_defineVariable_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::Parser::defineVariable(Storage object)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_Parser_variables_o, CX_DEFINED)) {
        Fast_Parser_variables_o->type = cx_resolve_ext(Fast_Parser_variables_o, NULL, "::cortex::lang::array{::cortex::Fast::Storage,64,::cortex::Fast::Storage}", FALSE, "element ::cortex::Fast::Parser::variables.type");
    ...
    if (!cx_checkState(Fast_Parser_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Parser_o)->parentState = 0x6;
    ...
    /* Define ::cortex::Fast::ParserDeclaration::storage */
    if (!cx_checkState(Fast_ParserDeclaration_storage_o, CX_DEFINED)) {
        Fast_ParserDeclaration_storage_o->type = cx_resolve_ext(Fast_ParserDeclaration_storage_o, NULL, "::cortex::Fast::Storage", FALSE, "element ::cortex::Fast::ParserDeclaration::storage.type");
        Fast_ParserDeclaration_storage_o->modifiers = 0x0;
        Fast_ParserDeclaration_storage_o->state = 0x6;
        Fast_ParserDeclaration_storage_o->weak = FALSE;
        Fast_ParserDeclaration_storage_o->id = 1;
        if (cx_define(Fast_ParserDeclaration_storage_o)) {
            cx_error("Fast_load: failed to define object '::cortex::Fast::ParserDeclaration::storage'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(Fast_ParserDeclaration_o, CX_DEFINED)) {
        ...
        cx_type(Fast_ParserDeclaration_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Ternary_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Ternary_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Unary_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Unary_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Update_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Update_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_Wait_o, CX_DEFINED)) {
        ...
        cx_type(Fast_Wait_o)->parentState = 0x6;
    ...
    if (!cx_checkState(Fast_While_o, CX_DEFINED)) {
        ...
        cx_type(Fast_While_o)->parentState = 0x6;
