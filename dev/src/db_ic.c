...
static cx_string cx_icValue_toString(cx_icValue operand, cx_string string) {
    ...
    case DB_IC_STORAGE:
        if (((cx_icStorage)operand)->kind == DB_STORAGE_ACCUMULATOR) {
...
static cx_bool cx_ic_validateSet(cx_icOp op) {
    cx_bool result = TRUE;
    op->s1 && (((cx_ic)op->s1)->kind != DB_IC_STORAGE) ? result = FALSE: 0;
    !op->s2 || (((cx_ic)op->s2)->kind != DB_IC_STORAGE) ? result = FALSE: 0;
...
static cx_bool cx_ic_validateCast(cx_icOp op) {
    ...
    op->s1 && (((cx_ic)op->s1)->kind != DB_IC_STORAGE) ? result = FALSE: 0;
    !op->s2 || (((cx_ic)op->s2)->kind != DB_IC_STORAGE) ? result = FALSE: 0;
    !op->s3 || (((cx_ic)op->s3)->kind != DB_IC_STORAGE) ? result = FALSE: 0;
    return result;
}
...
static cx_bool cx_ic_validateBinary(cx_icOp op) {
    ...
    if (result) {
        if ((((cx_ic)op->s2)->kind != DB_IC_STORAGE) && (((cx_ic)op->s3)->kind != DB_IC_STORAGE)) {
            result = FALSE;
        }
    }
    return result;
...
static cx_bool cx_icValidate(cx_icOp op) {
    ...
    switch(op->kind) {
            /* Set (assign) */
        case DB_IC_SET: result = cx_ic_validateSet(op); break;
            
            /* Cast */
        case DB_IC_CAST: result = cx_ic_validateCast(op); break;
        ...
        case DB_IC_WAIT: result = TRUE; break;
            ...
            /* Operators */
        case DB_IC_ADD: result = cx_ic_validateBinary(op); break;
        case DB_IC_SUB: result = cx_ic_validateBinary(op); break;
        case DB_IC_MUL: result = cx_ic_validateBinary(op); break;
        case DB_IC_DIV: result = cx_ic_validateBinary(op); break;
        case DB_IC_MOD: result = cx_ic_validateBinary(op); break;
        case DB_IC_INC: result = cx_ic_validateUnary(op); break;
        case DB_IC_DEC: result = cx_ic_validateUnary(op); break;
        case DB_IC_XOR: result = cx_ic_validateBinary(op); break;
        case DB_IC_OR: result = cx_ic_validateBinary(op); break;
        case DB_IC_AND: result = cx_ic_validateBinary(op); break;
        case DB_IC_NOT: result = cx_ic_validateUnary(op); break;
        case DB_IC_SHIFT_LEFT: result = cx_ic_validateBinary(op); break;
        case DB_IC_SHIFT_RIGHT: result = cx_ic_validateBinary(op); break;
        case DB_IC_STAGE1: result = TRUE; break;
        case DB_IC_STAGE2: result = TRUE; break;
        case DB_IC_COND_OR: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_AND: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_NOT: result = cx_ic_validateUnary
            (op); break;
        case DB_IC_COND_EQ: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_NEQ: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_GT: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_LT: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_GTEQ: result = cx_ic_validateBinary(op); break;
        case DB_IC_COND_LTEQ: result = cx_ic_validateBinary(op); break;
        ...
        case DB_IC_STOP: result = TRUE; break;
            ...
            /* Function calls */
        case DB_IC_PUSH: result = cx_ic_validatePush(op); break;
...
cx_icObject cx_icObject__create(cx_icProgram program, cx_uint32 line, cx_object ptr) {
    ...
    if (!(result = (cx_icObject)cx_icProgram_lookupStorage(program, name, TRUE))) {
        ...
        cx_icStorage_init((cx_icStorage)result, program, line, DB_STORAGE_OBJECT, name, t);
        result->ptr = ptr;
...
cx_icLocal cx_icLocal__create(cx_icProgram program, cx_uint32 line, cx_string name, cx_type type, cx_bool isParameter, cx_bool isReturn, cx_bool declare) {
    ...
    if (!(result = (cx_icLocal)cx_icProgram_lookupStorage(program, name, !declare))) {
        ...
        cx_icStorage_init((cx_icStorage)result, program, line, DB_STORAGE_LOCAL, name, type);
...
cx_icAccumulator cx_icAccumulator__create(cx_icProgram program, cx_uint32 line, cx_type type, cx_uint32 accumulatorId) {
    ...
    cx_icStorage_init((cx_icStorage)result, program, line, DB_STORAGE_ACCUMULATOR, name, type);
    cx_llAppend(program->scope->storages, result);
...
cx_icMember cx_icMember__create(cx_icProgram program, cx_uint32 line, cx_icStorage base, cx_member member) {
	...
	if (!(result = (cx_icMember)cx_icProgram_lookupStorage(program, name, TRUE))) {
		...
		cx_icStorage_init((cx_icStorage)result, program, line, DB_STORAGE_MEMBER, name, member->type->real);
		result->base = base;
...
cx_icElement cx_icElement__create(cx_icProgram program, cx_uint32 line, cx_type type, cx_icStorage base, cx_icValue index) {
    ...
    if (!(result = (cx_icElement)cx_icProgram_lookupStorage(program, name, TRUE))) {
        ...
        cx_icStorage_init((cx_icStorage)result, program, line, DB_STORAGE_ELEMENT, name, type);
        result->base = base;
        result->index = index;
...
cx_icDerefMode cx_icOp_derefMode(cx_icValue value) {
	...
	if (value && ((cx_ic)value)->kind == DB_IC_STORAGE) {
...
static void cx_icStorageFree(cx_icProgram _this, cx_icStorage storage, cx_uint32 line) {
    ...
    if (storage->isReference || ((type->kind == DB_PRIMITIVE) && (cx_primitive(type)->kind == DB_TEXT))) {
        cx_ic freeIc;
        
        /* Insert free-instruction */
        freeIc = (cx_ic)cx_icOp__create(_this, line, DB_IC_FREE, (cx_icValue)storage, NULL, NULL);
        if (storage->isReference) {
            ((cx_icOp)freeIc)->s1Deref = DB_IC_DEREF_ADDRESS;
        }
...
void cx_icProgram_scopePop(cx_icProgram _this, cx_uint32 line) {
    ...
    if (_this->scope->storages) {
        ...
        while(cx_iterHasNext(&storageIter)) {
            storage = cx_iterNext(&storageIter);
            if ((storage->kind == DB_STORAGE_LOCAL) && !((cx_icLocal)storage)->isReturn && !((cx_icLocal)storage)->isParameter) {
