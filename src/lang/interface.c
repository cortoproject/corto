...
corto_objectseq corto_interface_vtableFromBase(
...
corto_function* corto_vtableLookup(
    ...
    int32_t* d_out)
{
    ...
    result = (corto_function*)corto_lookup_functionFromSequence(
        s, member, d_out, NULL);
    ...
    corto_function method)
{
    ...
    if (i == vtable->length) {
        vtable->buffer = corto_realloc(
            vtable->buffer, sizeof(corto_member) * (vtable->length + 1));
        vtable->length++;
    }
    ...
    corto_method method)
{
    ...
    if (d == CORTO_OVERLOAD_ERROR) {
        ut_throw("method lookup error for '%s'", corto_idof(method));
        goto error;
    }
    ...
    if (procedureType == corto_override_o) {
        if (!found || !*found || d < 0) {
            if (found && *found) {
                ...
                ut_throw("no overridable method found for '%s'",
                    corto_fullpath(NULL, method));
            }
        ...
        {
            ut_throw("illegal override of '%s', method '%s' is not overridable ",
                corto_fullpath(NULL, method),
                corto_fullpath(NULL, *found));
            goto error;
        }
    ...
    if (found && (*found != (corto_function)method)) {
        ...
        if (!d) {
            ...
            if (corto_parentof(*found) != corto_parentof(method)) {
                ...
                if (strcmp(id, id2)) {
                    ut_throw("method '%s' conflicts with '%s'", id, id2);
                    goto error;
                }
            }
        }
    ...
    ut_assert(d != -1, "error occurred while looking up inserted method");
    ...
    corto_member m)
{
    ...
    if (corto_instanceof(corto_delegate_o, m->type) &&
        corto_instanceof(corto_parentof(m), this))
    {
        ...
        do {
            ...
            if (delegate->procedure &&
                delegate->procedure->kind == CORTO_PROCEDURE_STUB)
            {
                delegate = NULL;
            }

        } while ((!delegate || !delegate->procedure) &&
                 (base = corto_interface(base)->base));

        if (base && (base != this) &&
            corto_instanceof(corto_parentof(m), base))
        {
...
int16_t corto__interface_bindMember(
    corto_interface this,
    corto_member o)
{
...
uint16_t corto__interface_calculateAlignment(
    corto_interface this)
{
    uint32_t i;
    uint16_t alignment;
    ...
    for (i=0; i<this->members.length; i++) {
        ...
        if (memberAlignment) {
            ...
            ut_throw("member '%s' has type '%s' with zero-alignment.",
...
uint32_t corto__interface_calculateSize(
    corto_interface this,
    uint32_t base)
{
    uint32_t i, memberSize, size, alignment, interfaceAlignment;
    corto_member m;
    ...
    for (i=0; i<this->members.length; i++) {
        ...
        if (!corto_instanceof(corto_alias_o, m)) {
            ...
            if (!memberSize) {
                ut_throw("member '%s' of type '%s' is of invalid type '%s'",
                    corto_idof(m),
    ...
    corto_alias this)
{
    ...
    if (!this->member) {
        ut_throw("alias '%s' does not point to a member",
            corto_fullpath(NULL, this));
        goto error;
    } else {
        if (this->member == corto_member(this)) {
            ut_throw("alias member '%s' refers to itself",
                corto_fullpath(NULL, this));
            goto error;
        }

        if (super->modifiers & CORTO_HIDDEN) {
            ut_throw("invalid hidden modifier for alias '%s'",
                corto_fullpath(NULL, this));
            goto error;
        }
        ...
        if ((this->member->modifiers & ~CORTO_HIDDEN) != super->modifiers) {
            ut_throw("alias '%s' modifiers differ from member '%s'",
        ...
        if (!base) {
            ut_throw("alias '%s' points to member of another interface",
                corto_fullpath(NULL, this));
            goto error;
        }

        if (!(m & CORTO_HIDDEN)) {
            ut_throw("cannot alias non-hidden member '%s'",
                corto_fullpath(NULL, this->member));
            goto error;
        }

        if (m & (CORTO_PRIVATE)) {
            ut_throw("alias '%s' doesn't have write-access to member '%s'",
                corto_fullpath(NULL, this), corto_fullpath(NULL, this->member));
            goto error;
        }

        /* TODO: memory leak outside of bootstrap */
        corto_claim(this->member->type);
...
static
int corto_interface_insertMemberAction(
    void* o,
    void* userData)
{
    ...
    if (corto_class_instanceof(corto_member_o, o)) {
        ...
        if (!m->type) {
            ut_throw("member '%s' has no type", corto_fullpath(NULL, m));
            goto error;
        }
        ...
        if ((m->type->flags & CORTO_TYPE_HAS_REFERENCES) || m->type->reference){
        ...
        if (!m->type->reference && (m->type->flags & CORTO_TYPE_NEEDS_INIT)) {
            ...
            if (m->modifiers & CORTO_NOT_NULL ||
                m->type->kind == CORTO_COMPOSITE)
            {
    ...
    corto_interface this)
{
    ...
    ut_assert(this->next_member_id == this->members.length,
...
static
int16_t corto_interface_checkProcedureParameters(
    corto_function o1,
    corto_function o2)
{
    bool result;
    ...
    if (o1->parameters.length != o2->parameters.length) {
        ut_throw(
            "number of parameters does not match for "
            "functions '%s' (%d) and '%s' (%d)",
        ...
        for (i=0; i<o1->parameters.length; i++) {
            ...
            if (strcmp(o1->parameters.buffer[i].name,
            ...
            if (p1 != p2) {
                /* Perform a stricter check during bootstrap. This is because
                 * the corto_type_compatible method is not yet available during
                 * bootstrap, because the vtable of type is not yet final,
                 * resulting in unstable methodId's. */
                if (!corto_check_state(corto_type_o, CORTO_VALID)) {
                    /* If lang::type is not defined, we're bootstrapping. */
                    if (p1 != p2) {
                        if (!(p1 == corto_object_o) && (p2->reference)) {
                            ut_throw(
    "bootstrap failure: types of parameters '%s' and '%s' are not compatible.",
                                o1->parameters.buffer[i].name,
                                o2->parameters.buffer[i].name);
                            result = FALSE;
                        }
                ...
                } else if (!corto_type_castable(p1, p2)) {
                    /* This virtual function can only be called after the
                     * bootstrap is complete. */
                    ut_throw(
    "type of parameter %s of function '%s' is incompatible with function '%s'",
                ...
                o2->parameters.buffer[i].is_reference)
            {
                      ...
                      o1->parameters.buffer[i].is_reference))
                {
                    ...
                    ut_throw(
                        "parameter '%s' of function '%s' and '%s' has "
                        "conflicting pass-by-reference semantics.",
                        o1->parameters.buffer[i].name,
                        corto_fullpath(NULL, o1),
                        corto_fullpath(NULL, o2));
                    result = FALSE;
...
bool corto_interface_checkProcedureCompatibility(
    corto_function o1,
    corto_function o2)
{
    bool result;
    ...
    const char *name)
{
    ...
    if (dot && result) {
        if (result->type->kind == CORTO_COMPOSITE) {
            ...
            ut_throw("cannot resolve member '%s' on non-composite type '%s'",
                dot + 1,
    ...
    const char *name)
{
    ...
    if ((f = corto_vtableLookup(&this->methods, name, &d))) {
        result = ((corto_word)f -
            (corto_word)this->methods.buffer) / sizeof(corto_function);
    ...
    void *userData)
{
    ...
    if (ut_ll_count(m->tags)) {
        if (ut_ll_hasObject(m->tags, data->tag)) {
            data->result = m;
            return 1;
        }
    }
