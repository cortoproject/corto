...
int test_load(void) {
    ...
    /* Declare ::cortex::test */
    test_o = cx_declare(_o, "test", cx_type(cx_package_o));
    if (!test_o) {
    ...
    /* Define ::cortex::test */
    if (!cx_checkState(test_o, CX_DEFINED)) {
        test_o->url = NULL;
        if (cx_define(test_o)) {
            cx_error("test_load: failed to define object '::cortex::test'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(test_test_o, CX_DEFINED)) {
        ...
        cx_type(test_test_o)->parentState = 0x6;
    ...
    if (!cx_checkState(test_unit_o, CX_DEFINED)) {
        ...
        cx_type(test_unit_o)->parentState = 0x6;
    ...
    if (!cx_checkState(test_suite_o, CX_DEFINED)) {
        ...
        cx_type(test_suite_o)->parentState = 0x6;
