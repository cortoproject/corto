...
int os_load(void) {
    ...
    /* Declare ::cortex::os */
    os_o = cx_declare(_o, "os", cx_type(cx_void_o));
    if (!os_o) {
    ...
    os_loadAvgKind_o = cx_declare(os_o, "loadAvgKind", cx_type(cx_enum_o));
    if (!os_loadAvgKind_o) {
    ...
    os_loadAvgKind_MIN_1_o = cx_declare(os_loadAvgKind_o, "MIN_1", cx_type(cx_constant_o));
    ...
    os_loadAvgKind_MIN_15_o = cx_declare(os_loadAvgKind_o, "MIN_15", cx_type(cx_constant_o));
    ...
    os_loadAvgKind_MIN_5_o = cx_declare(os_loadAvgKind_o, "MIN_5", cx_type(cx_constant_o));
    ...
    /* Declare ::cortex::os::loadavg(loadAvgKind kind) */
    os_loadavg_o = cx_declare(os_o, "loadavg(loadAvgKind kind)", cx_type(cx_function_o));
    if (!os_loadavg_o) {
        cx_error("os_load: failed to declare object '::cortex::os::loadavg(loadAvgKind kind)'.");
        goto error;
    }

    /* Define ::cortex::os::loadavg(loadAvgKind kind) */
    if (!cx_checkState(os_loadavg_o, CX_DEFINED)) {
        os_loadavg_o->returnType = cx_resolve_ext(os_loadavg_o, NULL, "::cortex::lang::float64", FALSE, "element ::cortex::os::loadavg(loadAvgKind kind).returnType");
        os_loadavg_o->returnsReference = FALSE;
        
        /* Bind ::cortex::os::loadavg(loadAvgKind kind) with C-function */
        ...
        if (cx_define(os_loadavg_o)) {
            cx_error("os_load: failed to define object '::cortex::os::loadavg(loadAvgKind kind)'.");
            goto error;
        }
    }
