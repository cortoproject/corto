...
os_thread os_thread__new(void) {
    return cx_new(cx_type(os_thread_o));
}

os_thread os_thread__declare(cx_object _parent, cx_string _name) {
    return cx_declare(_parent, _name, cx_type(os_thread_o));
}
...
os_thread os_thread__create(void) {
    os_thread _this;
    _this = cx_new(cx_type(os_thread_o));
    if (cx_define(_this)) {
        cx_free(_this);
        _this = NULL;
    }
...
os_time os_time__new(void) {
    return cx_new(cx_type(os_time_o));
}

os_time os_time__declare(cx_object _parent, cx_string _name) {
    return cx_declare(_parent, _name, cx_type(os_time_o));
}
...
os_time os_time__create(cx_int32 seconds, cx_uint32 nanoseconds) {
    os_time _this;
    _this = cx_new(cx_type(os_time_o));
    _this->seconds = seconds;
    _this->nanoseconds = nanoseconds;
    if (cx_define(_this)) {
        cx_free(_this);
...
os_timer os_timer__new(void) {
    return cx_new(cx_type(os_timer_o));
}

os_timer os_timer__declare(cx_object _parent, cx_string _name) {
    return cx_declare(_parent, _name, cx_type(os_timer_o));
}
...
os_timer os_timer__create(cx_int32 sec, cx_uint32 nanosec) {
    os_timer _this;
    _this = cx_new(cx_type(os_timer_o));
    _this->sec = sec;
    _this->nanosec = nanosec;
    if (cx_define(_this)) {
        cx_free(_this);
