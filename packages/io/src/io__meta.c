...
int io_load(void) {
    ...
    /* Declare ::cortex::io */
    io_o = cx_declare(_o, "io", cx_type(cx_package_o));
    if (!io_o) {
    ...
    /* Define ::cortex::io */
    if (!cx_checkState(io_o, CX_DEFINED)) {
        io_o->url = NULL;
        if (cx_define(io_o)) {
            cx_error("io_load: failed to define object '::cortex::io'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(io_file_read_o, CX_DEFINED)) {
        cx_function(io_file_read_o)->returnType = cx_resolve_ext(io_file_read_o, NULL, "::cortex::lang::sequence{octet,0}", FALSE, "element ::cortex::io::file::read(uint32 bytes).returnType");
    ...
    if (!cx_checkState(io_file_readAll_o, CX_DEFINED)) {
        cx_function(io_file_readAll_o)->returnType = cx_resolve_ext(io_file_readAll_o, NULL, "::cortex::lang::sequence{octet,0}", FALSE, "element ::cortex::io::file::readAll().returnType");
    ...
    /* Declare ::cortex::io::file::write(sequence{octet,0} data) */
    io_file_write_o = cx_declare(io_file_o, "write(sequence{octet,0} data)", cx_type(cx_method_o));
    if (!io_file_write_o) {
        cx_error("io_load: failed to declare object '::cortex::io::file::write(sequence{octet,0} data)'.");
        goto error;
    }

    /* Define ::cortex::io::file::write(sequence{octet,0} data) */
    if (!cx_checkState(io_file_write_o, CX_DEFINED)) {
        cx_function(io_file_write_o)->returnType = cx_resolve_ext(io_file_write_o, NULL, "::cortex::lang::uint32", FALSE, "element ::cortex::io::file::write(sequence{octet,0} data).returnType");
        cx_function(io_file_write_o)->returnsReference = FALSE;
        io_file_write_o->virtual = FALSE;
        
        /* Bind ::cortex::io::file::write(sequence{octet,0} data) with C-function */
        ...
        if (cx_define(io_file_write_o)) {
            cx_error("io_load: failed to define object '::cortex::io::file::write(sequence{octet,0} data)'.");
            goto error;
        }
    }
    ...
    if (!cx_checkState(io_file_o, CX_DEFINED)) {
        ...
        cx_type(io_file_o)->parentState = 0x6;
