...
#define Fast_CHECK_ERRSET(parser) cx_assert(!parser->errSet, "%s:%d:%d: parser did not check error-status set on line %d", this->filename, this->line, this->column, this->errLine)
...
void Fast_Parser_error(Fast_Parser this, char* fmt, ... ) {
    ...
    Fast_reportError(this->filename, this->column ? line : line - 1, this->column, msgbuff, token);
...
void Fast_Parser_warning(Fast_Parser this, char* fmt, ... ) {
    ...
    Fast_reportWarning(this->filename, line, this->column, msgbuff, token);
...
cx_int16 Fast_Parser_toIc(Fast_Parser this) {
    ...
    if (Fast_Parser_finalize(this, program)) {
        goto error;
    }
...
typedef cx_object (*Fast_ExpandAction)(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData);

/* Combine results in new comma expression */
cx_object Fast_Parser_combineComma(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData) {
    CX_UNUSED(this);
    CX_UNUSED(userData);
...
cx_object Fast_Parser_combineConditionalExpr(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData) {
    ...
    if (lvalue) {
        ...
        Fast_Parser_collect(this, result);

        /* Fold expression where possible */
        result = Fast_Expression_fold(result);
        if (!result) {
            Fast_Parser_error(this, "folding failed for expression");
        }
    } else {
        result = rvalue;
    }
...
Fast_Expression Fast_Parser_createBinaryTernary(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, cx_operatorKind operator) {
...
cx_bool Fast_Parser_matchDelegateParameter(
    ...
    if ((t1 != t2) || (isRef1 && !isRef2)) {
        ...
        Fast_Parser_error(this, "%s %s '%s%s' of procedure doesn't match '%s%s' of delegate '%s'",
            name ? "parameter" : "",
...
Fast_Expression Fast_Parser_delegateAssignment(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue) {
    cx_uint32 i;
    ...
    if ((Fast_Node(rvalue)->kind == Fast_StorageExpr) && (Fast_Storage(rvalue)->kind == Fast_ObjectStorage)) {
        ...
        Fast_Parser_error(this, "dynamic delegate assignments are not yet supported");
        goto error;
    }
    ...
    if (type->parameters.length != tempCall->parameters.length) {
        Fast_Parser_error(this, "number of procedure parameters(%d) doesn't match delegate (%d)",
...
cx_object Fast_Parser_expandBinary(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData) {
    Fast_Expression result = NULL;
    ...
    if (tleft && (tleft->kind == CX_COMPOSITE) && (cx_interface(tleft)->kind == CX_DELEGATE)) {
        if (*(cx_operatorKind*)userData == CX_ASSIGN) {
            rvalue = Fast_Parser_delegateAssignment(this, lvalue, rvalue);
            if(!rvalue) {
                goto error;
            }
        } else {
            Fast_Parser_error(this, "operators other than assign not valid for delegates");
            goto error;
        }
    }
    ...
    if (Fast_Node(rvalue)->kind == Fast_TernaryExpr) {
        result = Fast_Parser_createBinaryTernary(this, lvalue, rvalue, operator);
...
cx_object Fast_Parser_expandMember(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData) {
    CX_UNUSED(userData);
    CX_UNUSED(this);
    Fast_Expression result = NULL;
...
/* Callback function for expansion of element expressions */
cx_object Fast_Parser_expandElement(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue, void *userData) {
    CX_UNUSED(this);
    CX_UNUSED(userData);
...
Fast_Expression Fast_Parser_explodeComma(Fast_Parser this, Fast_Expression lvalues, Fast_Expression rvalues, Fast_ExpandAction action, void *userData) {
    ...
    if ((rvalueCount > 1) && (lvalueCount == 1)) {
        ...
        if (Fast_Expression_hasSideEffects(Fast_Expression(cx_llGet(lvalueList,0)))) {
            if (Fast_Node(rvalues)->kind != Fast_InitializerExpr) {
                ...
                Fast_Parser_addStatement(this, Fast_Parser_binaryExpr(this, var, lvalues, CX_ASSIGN));
                rvalues = var;
            }
        }
    }
    ...
    Fast_ExpressionListForeach(lvalueList, l)
        Fast_ExpressionListForeach(rvalueList, r)
            Fast_Expression e = action(this, var ? var : l, r, userData);
            if (!e) {
                goto error;
            }
            Fast_Parser_collect(this, e);
...
Fast_Expression Fast_Parser_expandComma(Fast_Parser this, Fast_Expression lvalues, Fast_Expression rvalues, Fast_ExpandAction action, Fast_ExpandAction combine, void *userData) {
    ...
    if ((lvalueCount > 1) && (rvalueCount == 1)) {
        ...
        if (Fast_Expression_hasSideEffects(Fast_Expression(cx_llGet(rvalueList,0)))) {
            if (Fast_Node(rvalues)->kind != Fast_InitializerExpr) {
                ...
                Fast_Parser_addStatement(this, Fast_Parser_binaryExpr(this, var, rvalues, CX_ASSIGN));
                rvalues = var;
            }
        }
        ...
        Fast_Parser_error(this, "mismatch in expression count of binary operation (left = %d, right = %d)",
...
Fast_Expression Fast_Parser_resolve(Fast_Parser this, cx_id id) {
    Fast_Expression result = NULL;
    cx_object object;
    cx_char *ptr, ch;
    cx_id buffer;
    ...
    if (strchr(id, ':')) {
        ...
        object = cx_resolve(this->scope, id);
        if (!object){
            cx_object rvalueType = Fast_Parser_getLvalueType(this, FALSE);
    ...
    return result;
error:
    Fast_Parser_error(this, "unresolved identifier '%s'", id);
    fast_err;
    return NULL;
}

/* Create an observer */
Fast_Storage Fast_Parser_observerCreate(Fast_Parser this, cx_string id, Fast_Expression object, cx_eventMask mask, Fast_Object dispatcherVar) {
...
Fast_Storage Fast_Parser_declareDelegate(Fast_Parser this, cx_type returnType, cx_string id, cx_bool returnsReference) {
...
cx_void _Fast_Parser_addStatement(Fast_Parser this, Fast_Node statement) {
/* $begin(::corto::Fast::Parser::addStatement) */
    ...
    if (!this->pass && !statement) {
        if (this->stagingAllowed) {
            Fast_Parser_initDeclareStaged(this, NULL);
        }
    }
    ...
    if (!this->pass && statement) {
        if (statement->kind == Fast_CommaExpr) {
            Fast_Parser_error(this, "invalid usage of comma expression");
            goto error;
        }
    ...
    if (statement) {
        if (this->block && (Fast_Node(this->block->_while) != statement)) {
            if (this->block->_while) {
                Fast_Parser_error(this, "invalid statement after while");
                goto error;
            }
            if (!((statement->kind == Fast_LiteralExpr) && (Fast_Literal(statement)->kind == Fast_Text))) {
                Fast_Block_addStatement(this->block, statement);
                if (cx_instanceof(cx_type(Fast_Expression_o), statement)) {
                    if (!Fast_Expression_hasSideEffects(Fast_Expression(statement))) {
                        /* TODO: Fast_Parser_warning(this, "computed value is not used (%s)", cx_nameof(cx_typeof(statement))); */
                    }
                }
            } else if (this->pass) {
                /* If statement is a string, insert println function */
                if (!this->repl) {
                    Fast_Expression println = Fast_Parser_lookup(this, "io::println(string)");
                    Fast_Comma args = Fast_CommaCreate();
                    Fast_Comma_addExpression(args, Fast_Expression(statement));
                    Fast_Expression callExpr = Fast_Parser_callExpr(this, println, Fast_Expression(args));
                    Fast_Block_addStatement(this->block, Fast_Node(callExpr));
                    Fast_Parser_collect(this, args);
                } else {
                    Fast_Block_addStatement(this->block, statement);
                }
            }
...
cx_string _Fast_Parser_argumentToString(Fast_Parser this, cx_type type, cx_string id, cx_bool reference) {
/* $begin(::corto::Fast::Parser::argumentToString) */
    ...
    if (!type) {
        if (this->lastFailedResolve) {
            Fast_Parser_error(this, "unresolved type '%s'", this->lastFailedResolve);
        } else {
    ...
    if (!cx_class_instanceof(cx_type_o, type)) {
        cx_id id;
        Fast_Parser_error(this, "object '%s' used in parameter expression is not a type", Fast_Parser_id(type, id));
        goto error;
    }
...
Fast_Node _Fast_Parser_binaryExpr(Fast_Parser this, Fast_Expression lvalues, Fast_Expression rvalues, cx_operatorKind operator) {
/* $begin(::corto::Fast::Parser::binaryExpr) */
    ...
    if (lvalues && rvalues && (this->pass || ((this->initializerCount >= 0) && this->initializers[this->initializerCount]))) {
        ...
        case CX_ASSIGN_UPDATE: {
            ...
            Fast_Parser_addStatement(this, Fast_Node(result));
            Fast_Parser_collect(this, result);

            /* Insert assignment */
            if (!(result = Fast_Parser_binaryExpr(this, lvalues, rvalues, CX_ASSIGN))) {
                goto error;
            }
            Fast_Parser_addStatement(this, Fast_Node(result));
        ...
        default:
            if (!(result = Fast_Node(
                Fast_Parser_expandComma(this, lvalues, rvalues, Fast_Parser_expandBinary, combine, &operator)))) {
                goto error;
            }
            break;
        }
...
cx_int16 _Fast_Parser_bind(Fast_Parser this, Fast_Storage function, Fast_Block block) {
...
cx_int16 _Fast_Parser_bindOneliner(Fast_Parser this, Fast_Storage function, Fast_Block block, Fast_Expression expr) {
        ...
        returnLocal = Fast_Block_lookup(block, functionName);
         if (returnLocal) {
            ...
            if (!Fast_Expression(returnLocal)->type->reference && Fast_Expression(returnLocal)->isReference) {
                returnLocal = Fast_Parser_unaryExpr(this, returnLocal, CX_AND);
            }

            returnAssign = Fast_Expression(Fast_Parser_binaryExpr(this, returnLocal, expr, CX_ASSIGN));
            if (returnAssign) {
            ...
            Fast_Parser_error(this, "invalid assignment to function with no returntype");
            goto error;
        }

        /* Bind function */
        Fast_Parser_bind(this, function, block);
    }
...
cx_void _Fast_Parser_blockPop(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::blockPop) */
    ...
    if (this->pass) {
        if (this->block) {
            ...
            Fast_Parser_error(this, "unable to continue parsing due to previous errors");
            this->abort = TRUE;
        }
    }
...
Fast_Block _Fast_Parser_blockPush(Fast_Parser this, cx_bool presetBlock) {
/* $begin(::corto::Fast::Parser::blockPush) */
    ...
    if (this->pass) {
        ...
        if (!this->blockPreset || presetBlock) {
            newBlock = Fast_BlockCreate(this->block);
            Fast_Parser_collect(this, newBlock);
...
Fast_Expression _Fast_Parser_callExpr(Fast_Parser this, Fast_Expression function, Fast_Expression arguments) {
        ...
        if (functions) {Fast_ExpressionListForeach(functions, f)
            ...
            if (o && cx_instanceof(cx_type(cx_type_o), o)) {
                ...
                if (cx_llSize(exprs) != 1) {
                    Fast_Parser_error(this, "invalid number of parameters for cast (expected 1)");
                    Fast_Expression_cleanList(exprs);
                    goto error;
                }
                expr = Fast_Parser_castExpr(this, o, cx_llGet(exprs, 0));
                Fast_Expression_cleanList(exprs);
                Fast_Parser_collect(this, expr);
            } else {
        ...
        if (args) {Fast_ExpressionListForeach(args, a)
            if (Fast_Node(a)->kind == Fast_InitializerExpr) {
                ...
                if (Fast_Storage(var)->kind == Fast_TemporaryStorage) {
                    ...
                    Fast_Parser_collect(this, deinit);
                }
            }
        }}
...
Fast_Expression _Fast_Parser_castExpr(Fast_Parser this, cx_type lvalue, Fast_Expression rvalue) {
/* $begin(::corto::Fast::Parser::castExpr) */
    ...
    if (this->pass) {
        ...
        if (lvalue == rvalueType) {
            cx_id id;
            Fast_Parser_warning(this, "casting to value of the same type (%s)", Fast_Parser_id(lvalue, id));
            castRequired = FALSE;
        } else {
            if ((lvalue->kind == CX_COMPOSITE) && (rvalueType->kind == CX_COMPOSITE)) {
                if (cx_type_castable(lvalue, rvalueType)) {
                    cx_id id1, id2;
                    castRequired = FALSE;
                    Fast_Parser_warning(this, "upcasting from '%s' to '%s' does not require an explicit cast",
...
cx_void _Fast_Parser_collect(Fast_Parser this, cx_object o) {
...
cx_void _Fast_Parser_collectHeap(Fast_Parser this, cx_word addr) {
...
cx_int16 _Fast_Parser_construct(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::construct) */
    CX_UNUSED(this);
    Fast_Parser_reset(this);
    cx_threadTlsSet(Fast_PARSER_KEY, this);
    return 0;
/* $end */
}

/* ::corto::Fast::Parser::declaration(type type,string id,bool isReference) */
Fast_Storage _Fast_Parser_declaration(Fast_Parser this, cx_type type, cx_string id, cx_bool isReference) {
/* $begin(::corto::Fast::Parser::declaration) */
    ...
    if (this->blockCount || this->isLocal) {
        if (this->pass) {
            ...
            if (!this->block->parent) {
                ...
                if ((o = cx_resolve(this->scope, id))) {
                    Fast_Parser_error(this, "object '%s' is redeclared as a variable", id);
                    cx_release(o);
                    goto error;
                }
            }
            ...
            result = Fast_Storage(Fast_Block_declare(this->block, id, type, FALSE, isReference));
        ...
        if (!this->pass) {
            ...
            if (!o) {
                cx_id id1;
                Fast_Parser_error(this, "declare of '%s' of type '%s' failed",
                        id,
...
Fast_Storage _Fast_Parser_declareFunction(Fast_Parser this, cx_type returnType, cx_string id, cx_type kind, cx_bool returnsReference) {
    ...
    if (!this->pass) {
        ...
        if ((o = cx_lookup(this->scope, functionName))) {
            if (!cx_instanceof(cx_type(cx_function_o), o)) {
                cx_id id2;
                // todo changed here
                Fast_Parser_error(this, "cannot redeclare '%s' of type '%s' as function",
        ...
        if (!((function = cx_lookupFunction(this->scope, id, &distance)) && !distance)) {
            ...
            if (!result) {
                if (!cx_class_instanceof(cx_type_o, returnType)) {
                    cx_id id;
                    Fast_Parser_error(this, "object '%s' specified as returntype is not a type.", Fast_Parser_id(returnType, id));
                    goto error;
                }

                function = cx_declareChild(this->scope, id, functionType);
                if (!function) {
                    Fast_Parser_error(this, "declare of '%s' failed",
                                      functionName);
                    goto error;
                }
...
Fast_Block _Fast_Parser_declareFunctionParams(Fast_Parser this, Fast_Storage function) {
/* $begin(::corto::Fast::Parser::declareFunctionParams) */
    ...
    if (this->pass && function) {
        result = Fast_Parser_blockPush(this, TRUE);
        ...
        if (cx_procedure(cx_typeof(function_o))->kind == CX_METHOD) {
            ...
            if (!cx_instanceof(cx_type(cx_interface_o), parent)) {
                cx_id id;
                Fast_Parser_error(this, "parent of '%s' is not an interface nor of an interface type",
...
cx_int16 _Fast_Parser_define(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::define) */
    Fast_CHECK_ERRSET(this);

    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_define(this->initializers[this->initializerCount])) {
            goto error;
        }
...
cx_int16 _Fast_Parser_defineScope(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::defineScope) */
    Fast_CHECK_ERRSET(this);

    if (!this->pass) {
        if (!this->scope) {
            Fast_Parser_error(this, "invalid scope expression");
            goto error;
        }
        ...
        if (cx_instanceof(cx_type(cx_type_o), this->scope)) {
            if (cx_define(this->scope)) {
                cx_id id;
                Fast_Parser_error(this, "failed to define scope '%s'", Fast_Parser_id(this->scope, id));
                goto error;
            }
        } else {
            Fast_Define defineExpr = Fast_DefineCreate(Fast_Expression(Fast_ObjectCreate(this->scope)));
            Fast_Parser_addStatement(this, Fast_Node(defineExpr));
            Fast_Parser_collect(this, defineExpr);
        }
    }
...
cx_int16 _Fast_Parser_defineVariable(Fast_Parser this, Fast_Storage object) {
/* $begin(::corto::Fast::Parser::defineVariable) */
    ...
    if ((Fast_Node(object)->kind == Fast_StorageExpr) && (Fast_Storage(object)->kind == Fast_ObjectStorage)) {
        if (cx_define(Fast_Object(object)->value)) {
            cx_id id1, id2;
            Fast_Parser_error(this, "define of variable '%s' of type '%s' failed",
        ...
        Fast_Parser_collect(this, defineStmt);
        Fast_Parser_addStatement(this, Fast_Node(defineStmt));
    }
...
cx_void _Fast_Parser_destruct(Fast_Parser this) {
...
Fast_Expression _Fast_Parser_elementExpr(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue) {
    ...
    if (this->pass) {
        ...
        if (t) {
            if (t->kind == CX_COMPOSITE) {
                if (!(result = Fast_Parser_explodeComma(this, lvalue, rvalue, Fast_Parser_expandMember, NULL))) {
                    goto error;
                }
            } else {
                if (!(result = Fast_Parser_explodeComma(this, lvalue, rvalue, Fast_Parser_expandElement, NULL))) {
                    goto error;
                }
            }
        } else {
            Fast_Parser_error(this, "cannot resolve element, left-hand of expression has no type");
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_finalize(Fast_Parser this, ic_program program) {
...
cx_int16 _Fast_Parser_foreach(Fast_Parser this, cx_string loopId, Fast_Expression collection) {
...
cx_type _Fast_Parser_getComplexType(Fast_Parser this) {
...
Fast_Expression _Fast_Parser_getLvalue(Fast_Parser this, cx_bool assignment) {
...
cx_type _Fast_Parser_getLvalueType(Fast_Parser this, cx_bool assignment) {
/* $begin(::corto::Fast::Parser::getLvalueType) */
    ...
    if ((this->lvalueSp > 0) && this->lvalue[this->lvalueSp-1].expr) {
        result = Fast_Expression_getType(this->lvalue[this->lvalueSp-1].expr);
    }

    /* If no lvalue is set, check if there is an active initializer */
    if (!result) {
        if ((this->initializerCount >= 0) && (this->initializers[this->initializerCount])) {
            result = Fast_Initializer_currentType(this->initializers[this->initializerCount]);
        }
    }
...
Fast_Node _Fast_Parser_ifStatement(Fast_Parser this, Fast_Expression condition, Fast_Block trueBranch, Fast_If falseBranch) {
...
cx_void _Fast_Parser_initDeclareStaged(Fast_Parser this, Fast_Expression expr) {
/* $begin(::corto::Fast::Parser::initDeclareStaged) */
    ...
    if (expr) {
        ...
        if (this->stagedCount) {
            Fast_Parser_error(this, "invalid declaration");
            goto error;
        }
        ...
        for(i=0; i<this->stagedCount; i++) {
            if (cx_instanceof(cx_type(cx_type_o), this->scope)) {
                ...
                Fast_Parser_declaration(this, defaultType, this->staged[i].name, FALSE);
...
cx_int16 _Fast_Parser_initKeyValuePop(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initKeyValuePop) */
    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_popKey(this->initializers[this->initializerCount])) {
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_initKeyValuePush(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initKeyValuePush) */
    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_pushKey(this->initializers[this->initializerCount])) {
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_initKeyValueSet(Fast_Parser this, Fast_Expression expr) {
/* $begin(::corto::Fast::Parser::initKeyValueSet) */
    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_valueKey(this->initializers[this->initializerCount], expr)) {
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_initMember(Fast_Parser this, cx_string member) {
/* $begin(::corto::Fast::Parser::initMember) */
    Fast_CHECK_ERRSET(this);

    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_member(this->initializers[this->initializerCount], member)) {
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_initPop(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initPop) */
    Fast_CHECK_ERRSET(this);

    /* Primitive values are not pushed/pop'd */
    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_pop(this->initializers[this->initializerCount])) {
            goto error;
        }
    }
...
cx_int16 _Fast_Parser_initPush(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initPush) */
    Fast_CHECK_ERRSET(this);

    if (!this->variablePushed) {
        if ((this->initializerCount>=0) && this->initializers[this->initializerCount]) {
            if (Fast_Initializer_push(this->initializers[this->initializerCount])) {
                Fast_Parser_error(this, "invalid initializer scope");
                goto error;
            }
        }
    } else {
...
Fast_Expression _Fast_Parser_initPushExpression(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initPushExpression) */
    ...
    if (this->pass) {
        ...
        Fast_Parser_collect(this, initializer);
...
Fast_Expression _Fast_Parser_initPushIdentifier(Fast_Parser this, Fast_Expression type) {
/* $begin(::corto::Fast::Parser::initPushIdentifier) */
    ...
    if (!type) {
        if (this->lastFailedResolve) {
            Fast_Parser_error(this, "unresolved identifier '%s'", this->lastFailedResolve);
            goto error;
        } else {
            Fast_Parser_error(this, "invalid expression");
            goto error;
        }
    }
    ...
    if (!cx_instanceof(cx_type(cx_type_o), o)) {
        cx_id id;
        Fast_Parser_error(this, "invalid expression, '%s' is not a type", Fast_Parser_id(o, id));
        goto error;
    }
    ...
    if (this->initializerCount >= 0) {
        Fast_Initializer initializer = this->initializers[this->initializerCount];
        if (initializer) {
    ...
    if ((!this->pass && !isDynamic) || forceStatic) {
        ...
        this->initializers[this->initializerCount] = Fast_Initializer(Fast_StaticInitializerCreate(variables, 1));
        this->variablePushed = TRUE;
        ...
        Fast_Parser_collect(this, newExpr);
        if (!type_o->reference) {
            refVar = Fast_Parser_unaryExpr(this, var, CX_AND);
            Fast_Parser_collect(yparser(), refVar);
        }
        assignExpr = Fast_Expression(Fast_BinaryCreate(refVar, newExpr, CX_ASSIGN));
        Fast_Parser_collect(this, assignExpr);
        Fast_Parser_addStatement(this, Fast_Node(assignExpr));

        cx_setref(&variables[0].object, var);
        this->initializers[this->initializerCount] = Fast_Initializer(Fast_DynamicInitializerCreate(variables, 1, FALSE));
        this->variablePushed = TRUE;
    } else {
        cx_setref(&this->initializers[this->initializerCount], NULL);
        this->variablePushed = TRUE;
...
cx_int16 _Fast_Parser_initPushStatic(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::initPushStatic) */
    ...
    if (!isLocal && this->variableCount) {
        if (Fast_Storage(this->variables[0])->kind != Fast_ObjectStorage) {
            isLocal = TRUE;
        }
    }
    ...
    if (isLocal) {
        Fast_Parser_error(this, "usage of initializer invalid for local variables (use assignment instead)");
        goto error;
    }

    if (!this->variableCount) {
        Fast_Parser_error(this, "invalid initializer");
        goto error;
    }
    ...
    for(i=0; i<this->variableCount; i++) {
        cx_setref(&variables[i].object, Fast_Expression(this->variables[i]));
        variables[i].key = 0;
    ...
    if (!this->pass) {
        /* Create initializer */
        initializer = Fast_Initializer(Fast_StaticInitializerCreate(variables, this->variableCount));
        cx_setref(&this->initializers[this->initializerCount], initializer);
        Fast_Parser_collect(this, initializer);
    } else {
        /* Create dummy initializer */
        initializer = Fast_InitializerCreate(variables, this->variableCount);
        cx_setref(&this->initializers[this->initializerCount], initializer);
        Fast_Parser_collect(this, initializer);
    }
...
cx_void _Fast_Parser_initStage(Fast_Parser this, cx_string id, cx_bool found) {
...
cx_int16 _Fast_Parser_initValue(Fast_Parser this, Fast_Expression expr) {
/* $begin(::corto::Fast::Parser::initValue) */
    Fast_CHECK_ERRSET(this);

    if ((this->initializerCount >= 0) && this->initializers[this->initializerCount]) {
        if (Fast_Initializer_value(this->initializers[this->initializerCount], expr)) {
            goto error;
        }
    }
...
cx_bool _Fast_Parser_isAbortSet(Fast_Parser this) {
...
cx_bool _Fast_Parser_isErrSet(Fast_Parser this) {
...
Fast_Expression _Fast_Parser_lookup(Fast_Parser this, cx_string id) {
/* $begin(::corto::Fast::Parser::lookup) */
    ...
    if (this->pass) {
        ...
        cx_type complexType = Fast_Parser_getComplexType(this);
        if (complexType) {
        ...
        if (this->block && !result) {
            result = Fast_Expression(Fast_Block_resolve(this->block, id));
        }
    }
    if (!result) {
        result = Fast_Parser_resolve(this, id);
    }
    ...
    if (!result) {
        if (this->pass ||
            ...
            cx_instanceof(cx_type(Fast_StaticInitializer_o), this->initializers[this->initializerCount]))) {
            Fast_Parser_error(this, "unresolved identifier '%s'", id);
            fast_err;
        }
    ...
    Fast_Parser_initStage(this, id, result != NULL);
...
Fast_Expression _Fast_Parser_memberExpr(Fast_Parser this, Fast_Expression lvalue, Fast_Expression rvalue) {
/* $begin(::corto::Fast::Parser::memberExpr) */
    ...
    if (this->pass) {
        ...
        if (t) {
            ...
            Fast_Parser_error(this, "cannot resolve member, left-hand of expression has no type");
            goto error;
        }
    }
...
Fast_Storage _Fast_Parser_observerDeclaration(Fast_Parser this, cx_string id, Fast_Expression object, cx_eventMask mask, Fast_Object dispatcher) {
/* $begin(::corto::Fast::Parser::observerDeclaration) */
    ...
    if (!this->pass) {
        ...
        if (isTemplate) {
            result = Fast_Parser_observerCreate(this, id, object, mask, dispatcher);
            if (!result) {
                Fast_Parser_error(this, "failed to create template observer");
                goto error;
            }
        }
    } else {
        ...
        if (isTemplate) {
            ...
            block = Fast_Parser_blockPush(this, TRUE); /* Push new block on stack */

            /* If observer is not a template it has not yet been created, so create it now */
            result = Fast_Parser_observerCreate(this, id, object, mask, dispatcher);
            if (!result) {
                Fast_Parser_error(this, "failed to create observer");
                goto error;
            }
        ...
        Fast_Parser_bind(this, result, block);
...
cx_void _Fast_Parser_observerPush(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::observerPush) */
    ...
    if (this->pass) {
        if (cx_class_instanceof(cx_type_o, this->scope)) {
            Fast_Block block;
            block = Fast_Parser_blockPush(this, TRUE);
            Fast_Block_declareTemplate(block, "this", this->scope, TRUE, FALSE);
        }
    }
...
Fast_Expression _Fast_Parser_parseExpression(Fast_Parser this, cx_string expr, Fast_Block block, cx_object scope, cx_uint32 line, cx_uint32 column) {
            ...
            if (cx_llSize(block->statements) == 1) {
                ...
                if (cx_instanceof(cx_type(Fast_Expression_o), node)) {
                    ...
                    cx_print("%s: '%s' does not resolve to a valid expression (found %s).",
                            this->filename, expr, Fast_Parser_id(cx_typeof(node), id));
                    goto error;
                }
            } else {
...
cx_void _Fast_Parser_popComplexType(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::popComplexType) */

    this->complexTypeSp--;
    if (this->complexTypeSp < 0) {
        Fast_Parser_error(this, "parser error: complexType stack pointer below zero");
        abort();
    }
...
cx_void _Fast_Parser_popLvalue(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::popLvalue) */

    this->lvalueSp--;
    if (this->lvalueSp < 0) {
        Fast_Parser_error(this, "parser error: lvalue stack pointer below zero");
        abort();
    }
...
cx_void _Fast_Parser_popScope(Fast_Parser this, cx_object previous) {
...
Fast_Expression _Fast_Parser_postfixExpr(Fast_Parser this, Fast_Expression lvalue, cx_operatorKind operator) {
...
cx_void _Fast_Parser_pushComplexType(Fast_Parser this, Fast_Expression lvalue) {
/* $begin(::corto::Fast::Parser::pushComplexType) */

    if (lvalue) {
        cx_setref(&this->complexType[this->complexTypeSp], Fast_Expression_getType(lvalue));
    } else {
...
cx_void _Fast_Parser_pushLvalue(Fast_Parser this, Fast_Expression lvalue, cx_bool isAssignment) {
...
cx_int16 _Fast_Parser_pushPackage(Fast_Parser this, cx_string name) {
/* $begin(::corto::Fast::Parser::pushPackage) */
    char ch, *ptr, *bptr;
    cx_id buffer;

    if (this->scope && (this->scope != root_o)) {
        Fast_Parser_error(this, "#package may only be used in the root scope");
        goto error;
    }
    ...
    if (!memcmp(name, "::", 2)) {
        ...
        Fast_Parser_error(this, "packages must be specified using fully qualified names");
        goto error;
    }
        ...
        if (ch == ':') {
            if (ptr[1] == ':') {
                *bptr = '\0';
                ...
                if (!o) {
                    /* Declare package */
                    Fast_Parser_declaration(this, cx_type(cx_package_o), buffer, FALSE);

                    /* Push package as scope */
                    Fast_Parser_pushScope(this);

                    /* Define package */
                    Fast_Parser_defineScope(this);
                } else {
                ...
                Fast_Parser_error(this, "invalid package name '%s'", name);
                goto error;
            }
        } else {
            *bptr = ch;
...
cx_void _Fast_Parser_pushReturnAsLvalue(Fast_Parser this, cx_function function) {
/* $begin(::corto::Fast::Parser::pushReturnAsLvalue) */
    ...
    if (this->pass) {
        if (function->returnType) {
            ...
            result = Fast_Expression(Fast_Block_resolve(this->block, id));
            if (!result) {
                Fast_Parser_error(this, "parser error: can't find result variable '%s'", id);
                goto error;
            }
        }
    }

    Fast_Parser_pushLvalue(this, result, TRUE);
...
cx_object _Fast_Parser_pushScope(Fast_Parser this) {
/* $begin(::corto::Fast::Parser::pushScope) */
    ...
    if (this->variables[0]) {
        cx_setref(&this->scope, Fast_Object(this->variables[0])->value);
        Fast_Parser_reset(this);
    } else {
        goto error;
    }
...
cx_void _Fast_Parser_reset(Fast_Parser this) {
    ...
    if (this->pass) {
        for(i=0; i<this->stagedCount; i++) {
            if (!this->staged[i].found) {
                Fast_Parser_error(this, "unresolved identifier '%s'", this->staged[i].name);
                fast_err;
            }
            cx_dealloc(this->staged[i].name);
            this->staged[i].name = NULL;
...
Fast_Expression _Fast_Parser_ternaryExpr(Fast_Parser this, Fast_Expression cond, Fast_Expression iftrue, Fast_Expression iffalse) {
/* $begin(::corto::Fast::Parser::ternaryExpr) */
    Fast_Expression result = NULL;

    if (this->pass) {
        Fast_Expression lvalue = Fast_Parser_getLvalue(this, TRUE);
...
Fast_Expression _Fast_Parser_unaryExpr(Fast_Parser this, Fast_Expression lvalue, cx_operatorKind operator) {
    ...
    if (lvalue) {
        if (operator == CX_SUB) {
            ...
            if (lvalueType->kind == CX_PRIMITIVE) {
                ...
                case CX_FLOAT:
                    if (Fast_Node(lvalue)->kind == Fast_LiteralExpr) {
                        ...
                        if (this->pass) {
                            ...
                            Fast_Parser_collect(this, minusOne);
                ...
                default: {
                    cx_id id;
                    Fast_Parser_error(this, "unary operator - not applicable to type '%s'", Fast_Parser_id(lvalueType, id));
                    goto error;
                }
                }
            } else {
                cx_id id;
                Fast_Parser_error(this, "unary operator - not applicable to non-primitive type '%s'", Fast_Parser_id(lvalueType, id));
                goto error;
            }
            ...
            if (Fast_Node(lvalue)->kind == Fast_StorageExpr) {
                if (lvalue->isReference) {
                    if (cx_copy((cx_object*)&result, lvalue)) {
                        Fast_Parser_error(this, "parser error: failed to take reference (copy failed)");
                    }
                    result->deref = Fast_ByReference;
                    Fast_Node(result)->line = this->line;
                    Fast_Node(result)->column = this->column;
                    ...
                    Fast_Parser_error(this, "cannot take reference from non-reference variable");
                    goto error;
                }
            } else {
...
Fast_Node _Fast_Parser_updateStatement(Fast_Parser this, Fast_Expression expr, Fast_Block block) {
        ...
        if (functionBlock) {
            ...
            if (functionBlock) {
                if ((procedureKind == CX_METHOD) || ((procedureKind == CX_OBSERVER) && cx_observer(function)->template)) {
                    from = Fast_Parser_lookup(this, "this");
                }
            }
        }
...
Fast_Expression _Fast_Parser_waitExpr(Fast_Parser this, Fast_Expression expr, Fast_Expression timeout) {
...
Fast_Node _Fast_Parser_whileStatement(Fast_Parser this, Fast_Expression condition, Fast_Block trueBranch, cx_bool isUntil) {
