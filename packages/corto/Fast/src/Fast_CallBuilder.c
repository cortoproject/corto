...
Fast_Call Fast_CallBuilder_buildMethod(Fast_CallBuilder *this) {
    Fast_Call result = NULL;
    cx_member m;
    cx_function f;
    cx_type t = Fast_Expression_getType(this->instance);
    ...
    if (t->kind == CX_COMPOSITE) {
        ...
        if (m) {
            Fast_String nameExpr = Fast_StringCreate(this->name);
            Fast_Parser_collect(yparser(), nameExpr);
            if ((m->type->kind == CX_COMPOSITE) && (cx_interface(m->type)->kind == CX_DELEGATE)) {
                Fast_Expression fExpr = Fast_Expression(Fast_MemberCreate(this->instance, Fast_Expression(nameExpr)));
                if (!fExpr) {
                    goto error;
                }
                result = Fast_Call(Fast_DelegateCallCreate(NULL, this->arguments, fExpr));
            }
        }
    }
    ...
    if (!result) {
        ...
        result = Fast_Call(Fast_StaticCallCreate(this->instance, this->arguments, f));
    }
...
Fast_Call _Fast_CallBuilder_build(Fast_CallBuilder *this) {
/* $begin(::corto::Fast::CallBuilder::build) */
    Fast_Call result = NULL;

    /* Obtain signature */
    if (Fast_CallBuilder_buildSignature(this)) {
        goto error;
    }

    /* If instance is explicitly set call must be a named callable (method, metaprocedure) */
    if (this->instance) {
        result = Fast_CallBuilder_buildMethod(this);
    } else {
        /* If block is set, test for callable variables */
        if (this->block) {
            Fast_Local l = Fast_Block_resolveLocal(this->block, this->name);
            if (l) {
                /* Check if l is of a delegate type */
                if ((l->type->kind == CX_COMPOSITE) && (cx_interface(l->type)->kind == CX_DELEGATE)) {
                    result = Fast_Call(Fast_DelegateCallCreate(NULL, this->arguments, Fast_Expression(l)));
                    if (!result) {
                        goto error;
                    }
                }
            ...
            } else if ((l = Fast_Block_resolveLocal(this->block, "this"))) {
        ...
        if (!result) {
            ...
            if (!f) {
                ...
                Fast_Parser_error(yparser(), "no procedure found with signature '%s' in scope '%s'", 
                    this->signature,
                    Fast_Parser_id(this->scope, id));
                goto error;
            }
            if ((cx_typeof(f)->kind == CX_COMPOSITE) && (cx_interface(cx_typeof(f))->kind == CX_DELEGATE)) {
                Fast_Expression fExpr = Fast_Expression(Fast_ObjectCreate(f));
                result = Fast_Call(Fast_DelegateCallCreate(NULL, this->arguments, fExpr));   
            } else {
                ...
                result = Fast_Call(Fast_StaticCallCreate(NULL, this->arguments, f));
            }  
        }
    }
...
cx_int16 _Fast_CallBuilder_buildSignature(Fast_CallBuilder *this) {
/* $begin(::corto::Fast::CallBuilder::buildSignature) */
    cx_string signature;
