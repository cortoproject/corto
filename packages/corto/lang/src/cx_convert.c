...
CX_DECL_TRANSFORM(string, boolean) {
    ...
    if (!strcmp(str, "TRUE") || !strcmp(str, "true")) {
        ...
        *(cx_bool*)to = FALSE;
        ...
        cx_seterr("'%s' is not a valid boolean value", str);
        return -1;
    }
    return 0;
}
...
cx_int16 cx_convert(cx_primitive fromType, void *from, cx_primitive toType, void *to) {
    ...
    if (c) {
        ...
        cx_id id1, id2;
        cx_seterr("no conversion possible from primitive type '%s' to '%s'.", cx_fullname(fromType, id1), cx_fullname(toType, id2));
        goto error;
    }
